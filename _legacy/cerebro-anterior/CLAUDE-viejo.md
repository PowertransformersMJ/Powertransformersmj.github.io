...
- **Deploys (flujo ADR-005, desde 2026-06-06): YO ejecuto los `firebase deploy`** (firebase CLI local autenticado en la Mac del director) al tocar `firestore.rules` → `--only firestore:rules`; `firestore.indexes.json` → `--only firestore:indexes`; `storage.rules` → `--only storage`; `functions/*` → `--only functions`. **Anunciar el deploy en el MISMO turno** (acción de producción) y verificar el resultado. Detalle en `docs/30-LECCIONES.md` (L-09) + `99 §5` (ADR-005).
- **Commits y push (flujo ADR-005)**: **YO creo los commits** (`git add` de archivos específicos, NUNCA `Debug/` ni secretos; footer `Co-Authored-By`). **El director hace los `git push`** (GitHub Desktop / terminal). **NUNCA hago force-push a `main`** — lo dispara el director. El push del runtime da 403 (L-01); por eso publica el director. NUNCA escribir tokens a archivo/commit/log.
