...
export function clasificarAccion(s) {
  ...
  if (/(PLAN DE INVERSION|\bPI\b|AUMENTO DE CAPACIDAD DE TRANSFORMACION|INSTALACION (DE )?UNIDAD|REPOSICION|REEMPLAZO (DEL? |DE LA )?(TRANSFORMADOR|UNIDAD|ACTIVO))/.test(u)) return 'INV';
  ...
  if (/(CORRECCION DE FUGAS|MANTENIMIENTO (PREVENTIVO )?OLTC|ACTUALIZACION DE (ACCESORIOS|TABLERO)|RETROFIT|PINTURA|REEMPLAZO|REPARACION|\bMANTENIMIENTO\b)/.test(u)) return 'CORR';
  return 'DIAG';
}
