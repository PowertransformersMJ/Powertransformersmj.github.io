// ══════════════════════════════════════════════════════════════
// SGM · TRANSPOWER — Dominio puro: agregación de KPIs del tablero
// ──────────────────────────────────────────────────────────────
// POR QUÉ EXISTE ESTE MÓDULO
// `computeFromDatasets` es cálculo puro —recibe los datasets ya
// cargados y devuelve el snapshot del tablero— pero vivía dentro de
// `data/kpis.js`, que importa Firebase por URL https. Consecuencia:
// Node no podía importarlo, y los tests que lo cubrían se saltaban
// solos con un `catch { return }`. Pasaban en verde SIN ejecutar un
// solo assert: los KPIs de salud del parque (promedio HI, vida
// remanente, FUR pendientes, join-guard de órdenes huérfanas)
// figuraban como probados y no lo estaban. Peor que no tener test,
// porque daba falsa confianza.
//
// Al vivir aquí el cálculo se prueba de verdad y `data/kpis.js` se
// queda con lo que sí es su trabajo: leer de Firestore. La función
// se re-exporta desde allí, así que ningún llamador cambia (ADR-063).
//
// Sin dependencias de Firestore: solo enumeraciones de dominio.
// ══════════════════════════════════════════════════════════════

import { DEPARTAMENTOS, departamentoLabel } from './schema.js';
import {
  TIPOS_ORDEN, PRIORIDADES, ESTADOS_ORDEN_V1 as ESTADOS_ORDEN
} from './orden_schema.js';
