...
| `d3d8513` | fix | **Carga de informes solo desde Biblioteca + arregla 6→3.** Se retira la sección `#carga` del Tablero (la carga vive solo en Biblioteca). `mergeInformes` ya no deduplicaba por año: ahora conserva todos los informes vivos distintos y solo descarta los del seed cuyo año ya cubre un informe vivo, así los 6 PDFs subidos de una serie se ven todos. |
| _(este)_ | feat | **Reprocesar informes ya cargados (sin re-subir).** Los informes subidos antes de que existiera el OCR quedaron "pendiente de extracción"; ahora cada uno muestra un botón **↻ Reprocesar** (solo en informes vivos no-base con archivo). Al pulsarlo el shell descarga el PDF/imagen ya almacenado (`fetch(downloadURL)` → `File` con su `contentType`), lo pasa por la ruta común `leerTextoArchivo` (texto / OCR escaneo / OCR imagen) y actualiza el informe **en sitio** con `actualizarInforme` (sin borrar ni volver a subir). El núcleo OCR se factorizó en `leerTextoArchivo(file, setEstado) → {texto, ocr}`, reutilizado por la carga y por el reprocesado. El extractor sigue siendo conservador; la serie se confirma con `confirmarSerie`. Resuelve "aún siguen en el mismo estado". |
| `9949be5` | feat | **Extracción por OCR para escaneos e imágenes.** `extraerTexto` reescrito con 3 rutas: PDF con capa de texto (`pdf.js`), PDF escaneado (capa pobre `<60` alfanuméricos → render a canvas `escala 2.2` + OCR Tesseract.js `spa`) e imagen (OCR directo). Carga perezosa del motor OCR desde CDN; worker reutilizado y liberado al cerrar (`liberarOCR`). El modal admite ahora `image/*`, muestra estado por archivo (`.fi-status`) y bloquea "Continuar" mientras un informe se lee. `subirPDF` usa el `contentType` real. El extractor de mediciones sigue siendo conservador: cada prueba conserva su dato correcto, vacío antes que equivocado. Si el informe no trae serie, se acepta la tecleada por el operador. |
| _(este)_ | feat | **Familias de prueba múltiples · DRM del conmutador (OLTC).** No todas las series reciben las mismas pruebas: el modelo de informe gana un discriminador `tipo_prueba` (`predictivo_completo` / `tan_delta` / `drm_oltc` / `resistencia_devanados` / `ttr` / `mixto`, inferido de las mediciones presentes) y un bloque nuevo `drm` con la identidad del conmutador (fabricante, tipo, serial, posiciones, operaciones, posición nominal, tensión/corriente Ui, R de conmutación) y la ventana de tiempos de transición. Calificador normativo `calificarDrm(min,max)` sobre la ventana **40–70 ms** (guías ámbar 45/65 · IEEE C57.152-2013): dentro de [45,65] verde, entre guía y límite ámbar, fuera de [40,70] rojo. Cadena completa dominio → UI (fila DRM en la matriz semáforo, tabla resumen + identidad del conmutador, gráfica de banda `c-drm`) → HTML (`#drm` + fila de criterios). Seed: se anexa **un** informe real **2025** de la serie `173523-15510` que consolida los reportes reales **MPT045 (DRM · OLTC)** y **MPT044 (TTR)** de mayo/2025 — solo valores de resumen reales (conmutador MR V III 200 Y-76 serial 145981, ventana 56–66 ms); el detalle por fase no se publica en el reporte, así que `transiciones` queda vacío y la resistencia se marca `verificar` (mejor vacío que inventado). +27 tests (semáforo DRM + sanitizado/validación DRM + seed-2025). |
| _(prev)_ | feat | **Gating por número de serie.** Todo el interior del módulo se ilustra solo después de elegir una serie en un `<select id="serieSelect">` que gobierna ambas pestañas (colocado entre `page-header` y `.sgm-tabs`, scoped en `.pe-scope`). Por defecto NO hay selección: matriz, identidad, tablas, gráficas e historial muestran un prompt ("Selecciona un número de serie…") y los KPIs por unidad (`kpi-informes`/`kpi-estado`) quedan en "—". El controlador deja de auto-seleccionar la primera unidad (`arrancar` ya no llama `seleccionarUnidad(state.unidades[0])`): nuevas `renderVacioSeleccion`, `poblarSelectorSerie`, `sincronizarSeleccion` + handler `change` del select. Las tarjetas del parque (`.det`, ahora `<button data-serie>`) también seleccionan su serie. Los KPIs de flota (`kpi-unidades`/`kpi-subestaciones`) y la grilla del parque siguen visibles siempre. Sin `<datalist>` (§0.1.2.12). Para esta unidad la serie es `173523-15510`. |
