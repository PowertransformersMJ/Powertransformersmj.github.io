// ══════════════════════════════════════════════════════════════
// DEV · Tablas de valores POR PRUEBA (alto nivel) — preview aislado
// ──────────────────────────────────────────────────────────────
// Replica el estilo de la tabla-RESUMEN de corriente de excitación
// (banda por NIVEL/DEVANADO + KPI tiles + franja de norma + tabla por
// informe con criterio) y lo extiende a TODAS las pruebas presentes en
// los informes. Discrimina por nivel de tensión de prueba / devanado /
// configuración, y deriva la calificación del VALOR vs NORMA (nunca del
// texto del laboratorio · L-36).
//
// NO toca producción: reutiliza en SOLO LECTURA `derivarTablaTAP` y los
// helpers de `pruebas_electricas_bloques.js`, y las clases CSS `pe-fus-*`
// / `pe-tabla` de `pruebas-electricas.css`. Toda la lógica de armado vive
...
// skills, sensible a la tendencia. Se REUSA (no se reinventa).
import { accionPrueba } from '../assets/js/domain/pruebas_electricas_recomendaciones.js';
// REUSO la lógica de NIVEL DE TENSIÓN de producción (no reinventar): el "kV" del
// título de excitación suele ser la tensión de ENSAYO (10 kV), NO el nivel del
// devanado — el nivel real (AT 66/110 · MT 34.5 · BT 13.8) se deriva de DEVANADO +
// CONFIG vía `nivelDe`. (Causa raíz del bug: yo había reimplementado esto con regex.)
import { nivelDe, perdidasDe } from '../assets/js/ui/pruebas/excitacion-panel.js';
...
function grupoDe(familia, titulo, config) {
...
const diagBadge = (e) => {
  if (!e || e.nivel < 0) return `<span class="pe-diag-badge">${e ? esc(e.etiqueta) : 'sin dato'}</span>`;
  const cls = e.nivel === 0 ? 'is-ok' : e.nivel >= 3 ? 'is-bad' : 'is-warn';
  return `<span class="pe-diag-badge ${cls}">${esc(e.etiqueta)}</span>`;
};
...
function metricaFila(familia, f) {
  const crit = CRIT[familia];
...
function tendenciaFilas(familia, filas) {
  const crit = CRIT[familia];
...
function peorCasoFila(familia, filas) {
  const crit = CRIT[familia];
...
function bloqueDiagnostico(familia, filas) {
  ...
  const wrap = document.createElement('div'); wrap.className = 'pe-diag';
  let html = '<div class="pe-diag-h">Diagnóstico conforme a norma</div>';
  html += `<div class="pe-diag-metric">Métrica peor caso: <b>${fmt(metrica)} ${esc(unidadMetrica(familia, filas))}</b> · ${esc(DESC_METRICA[familia] || 'métrica peor caso')}</div>`;
  if (ev) {
    html += '<table class="pe-diag-tbl"><thead><tr><th>Norma</th><th>Criterio / umbral</th><th>Veredicto</th></tr></thead><tbody>';
    ev.opticas.forEach((o) => {
      ...
      html += `<tr><td>${sello}</td><td>${esc(o.umbral)}${o.nota ? ` <span class="pe-diag-nota">(${esc(o.nota)})</span>` : ''}</td><td>${diagBadge(o.estado)}</td></tr>`;
    });
    html += '</tbody></table>';
    html += `<div class="pe-diag-cons">Consolidado (criterio conservador): ${diagBadge(ev.consolidado)}${ev.divergen ? ' <span class="pe-diag-nota">· las normas divergen (esa divergencia ES información)</span>' : ''}</div>`;
    html += `<div class="pe-diag-concl">${esc(conclusionDiag(ev.consolidado))}</div>`;
    ...
    html += '<div class="pe-diag-h" style="margin-top:12px">Análisis y recomendación (según resultados)</div>';
    html += `<div class="pe-diag-metric">Resultados: <b>${filas.length}</b> informe(s) evaluado(s) · peor caso <b>${fmt(pc.metrica)} ${esc(u)}</b>${pc.fila ? ` en ${esc(pc.fila.label)}${pc.fila.config ? ' (' + esc(pc.fila.config) + ')' : ''}` : ''} · tendencia <b>${esc(trendTxt)}</b>.</div>`;
    html += `<div style="margin-top:6px"><span class="pe-diag-badge ${ACC_CLS[acc.clase] || ''}">${esc(acc.etiqueta)}</span> <span style="font-size:12px;color:#0f172a">${esc(acc.texto)}</span></div>`;
    if (familia === 'excitacion') {
      html += '<div class="pe-diag-nota" style="margin-top:6px">Pérdidas (W): componente resistiva del ensayo (I_exc = magnetizante + pérdidas) — se evalúan por <b>COMPARACIÓN</b> (vs fábrica / ensayos previos / fases hermanas), sin umbral % duro propio (NETA/IEEE comparativo, L-53).</div>';
    }
  } else {
    html += '<div class="pe-diag-concl">Prueba comparativa — sin umbral pasa/no-pasa; se interpreta vs huella/baseline.</div>';
  }
  wrap.innerHTML = html;
  return wrap;
}

// ── Tarjeta de un GRUPO (nivel/devanado/config) para una prueba ──
function tarjetaGrupo(familia, grupo, filas, withDiag = false) {
  const crit = CRIT[familia];
  ...
  band.innerHTML = `<span class="pe-fus-niv">${esc(grupo)}</span>`;
  band.appendChild(badgeEstado(estadoGrupo)); card.appendChild(band);

  // KPI tiles — universales (f.valores es un array de números en CUALQUIER modo).
  const todos = filas.flatMap((f) => f.valores).filter((v) => v != null);
  ...
  if (crit.kind === 'desb') {
    ...
    const lim = crit.limite;
    const dc = desvMax == null ? '' : Math.abs(desvMax) > lim ? 'is-bad' : Math.abs(desvMax) > lim * 0.7 ? 'is-warn' : 'is-ok';
    tiles.innerHTML = tile(fmt(valMax), ' ' + (filas[0].unidad || ''), 'Valor máx') +
      tile(fmt(desvMax), ' %', 'Δ / desv. máx', dc) +
      tile(filas.length, '', 'Informes');
  } else if (crit.kind === 'min') {
    const mn = Math.min(...todos);
    tiles.innerHTML = tile(fmt(mn), ' ' + crit.unidad, 'Mínimo', estadoMin(mn, crit.min) === 'ok' ? 'is-ok' : 'is-warn') +
      tile(fmt(Math.max(...todos)), ' ' + crit.unidad, 'Máximo') +
      tile(filas.length, '', 'Informes');
  } else {
    const mx = Math.max(...todos);
    const dc = mx > crit.limite ? 'is-bad' : mx > crit.guia ? 'is-warn' : 'is-ok';
    tiles.innerHTML = tile(fmt(mx), ' ' + crit.unidad, 'Máximo', dc) +
      tile(filas.length, '', 'Informes') +
      tile(todos.length, '', 'Mediciones');
  }
  card.appendChild(tiles);

  // Franja de norma + veredicto del grupo.
  ...
  const vb = document.createElement('span'); vb.className = 'pe-fus-vb'; vb.appendChild(badgeEstado(estadoGrupo));
  vr.appendChild(vb); card.appendChild(vr);
  ...
  // La columna de desviación SOLO aplica a pruebas de desbalance entre fases
  // (excitación / relación / resistencia). Para tan δ / bujes (criterio = valor)
  // o aislamiento (criterio = mínimo) no tiene sentido y confundiría.
  const mostrarDesv = esFases && crit.kind === 'desb';

  let headCols;
  if (esFases) {
    headCols = [...filas[0].faseNames, ...(mostrarDesv ? ['Desv. máx %'] : [])];
  } else {
    // Unión de etiquetas de ítem en orden de aparición (alinea columnas entre años).
    headCols = [];
    filas.forEach((f) => f.colLabels.forEach((l) => { if (!headCols.includes(l)) headCols.push(l); }));
  }
  ...
  filas.forEach((f) => {
    ...
    if (esFases) {
      f.faseAvg.forEach((v, i) => { cells += `<td${i === 1 ? ' class="col-b"' : ''}>${fmt(v)}</td>`; });
      if (mostrarDesv) {
  ...
  if (withDiag) card.appendChild(bloqueDiagnostico(familia, filas));
  return card;
}

// ── Sección de una PRUEBA (todas sus tarjetas de grupo) ──
function seccionFamilia(familia, informes) {
  const crit = CRIT[familia];
  ...
  informes.forEach((inf) => {
    (inf.bloques || []).forEach((b) => {
      ...
      // Extracción UNIFORME para ambos modos.
      let valores, desvMax = null, faseNames = null, faseAvg = null, colLabels = null, valoresRaw = null;
      if (a.mode === 'fases') {
        faseNames = a.faseNames; faseAvg = a.faseAvg;
        valores = a.faseAvg.slice();
        desvMax = a.desvMax;
      } else {
        colLabels = a.items.map((i) => i.label);
        valores = a.items.map((i) => i.value);
        valoresRaw = a.items;
        if (crit.kind === 'desb') desvMax = desvItems(a.items);
      }
      const vnum = valores.filter((v) => v != null);
      let estado;
      if (crit.kind === 'desb') estado = estadoDesb(desvMax, b.limite_desbalance != null ? b.limite_desbalance : crit.limite);
      ...
      grupos.get(g).push({
        mode: a.mode, label: inf.label, config: inf.config, ano: inf.ano, unidad: b.unidad,
        faseNames, faseAvg, colLabels, valores, valoresRaw, desvMax, estado
      });
    });
  });
  ...
  const h = document.createElement('div'); h.className = 'pe-sec';
  h.innerHTML = `${esc(NOMBRE[familia])} <small>· ${grupos.size} grupo(s) de discriminación · calificación DERIVADA del valor vs norma</small>`;
  sec.appendChild(h);

  const root = document.createElement('div'); root.className = 'pe-fus';
  [...grupos.entries()].sort((a, b) => a[0].localeCompare(b[0])).forEach(([g, filas]) => {
    filas.sort((a, b) => (a.ano || 0) - (b.ano || 0));
    root.appendChild(tarjetaGrupo(familia, g, filas));
  });
  sec.appendChild(root);
  return sec;
}

// ── VISTA A: todas las pruebas apiladas en una sola página ──
export function montarTablasPruebas(cont, informes) {
  cont.innerHTML = '';
  let pintadas = 0;
  ORDEN.forEach((familia) => {
    const sec = seccionFamilia(familia, informes);
    if (sec) { cont.appendChild(sec); pintadas++; }
  });
  if (!pintadas) {
    cont.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: 'Sin pruebas tabulables en los informes cargados.' }));
  }
  return pintadas;
}

// ── VISTA B: una PESTAÑA por prueba (se ve una prueba a la vez) ──
export function montarTablasPorPrueba(cont, informes) {
  cont.innerHTML = '';
  const secs = new Map();
  ORDEN.forEach((familia) => { const s = seccionFamilia(familia, informes); if (s) secs.set(familia, s); });
  if (!secs.size) {
    cont.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: 'Sin pruebas tabulables en los informes cargados.' }));
    return 0;
  }
  const tabbar = document.createElement('div'); tabbar.className = 'pe-tabs';
  const body = document.createElement('div');
  const keys = [...secs.keys()];
  const show = (familia) => {
    body.innerHTML = ''; body.appendChild(secs.get(familia));
    [...tabbar.children].forEach((b) => b.classList.toggle('is-active', b.dataset.fam === familia));
  };
  keys.forEach((familia) => {
    const b = document.createElement('button');
    b.className = 'pe-tab'; b.dataset.fam = familia; b.textContent = NOMBRE[familia];
    b.onclick = () => show(familia);
    tabbar.appendChild(b);
  });
  cont.appendChild(tabbar); cont.appendChild(body);
  show(keys[0]);
  return secs.size;
}

// ── VISTA "por AÑO": una PESTAÑA por INFORME (fecha + config); al elegirla se
// ven TODAS las pruebas tabuladas de ESE informe. NO se colapsan dos informes
// del mismo año (identidad por INFORME, no por año). ──
export function montarTablasPorInforme(cont, informes) {
  cont.innerHTML = '';
  const ordenados = informes.slice().sort((a, b) =>
    (a.ano || 0) - (b.ano || 0) || String(a.label).localeCompare(String(b.label)));
  if (!ordenados.length) {
    cont.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: 'Sin informes cargados.' }));
    return 0;
  }
  const tabbar = document.createElement('div'); tabbar.className = 'pe-tabs';
  const body = document.createElement('div');
  const render = (inf, btn) => {
    body.innerHTML = '';
    let pintadas = 0;
    ORDEN.forEach((familia) => { const s = seccionFamilia(familia, [inf]); if (s) { body.appendChild(s); pintadas++; } });
    if (!pintadas) body.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: 'Este informe no trae pruebas tabulables.' }));
    [...tabbar.children].forEach((b) => b.classList.toggle('is-active', b === btn));
  };
  ordenados.forEach((inf) => {
    const b = document.createElement('button');
    b.className = 'pe-tab';
    b.textContent = inf.label + (inf.config ? ' · ' + inf.config : '');
    b.onclick = () => render(inf, b);
    tabbar.appendChild(b);
  });
  cont.appendChild(tabbar); cont.appendChild(body);
  render(ordenados[0], tabbar.children[0]);
  return ordenados.length;
}

// ── VISTA "B + filtro de AÑO": pestañas por PRUEBA (arriba) + filtro de AÑO
// (abajo). Eliges la prueba y un año → la tabla de ESA prueba se acota a los
// valores + calificación de ESE año ("Todos" = todos los informes). ──
export function montarTablasPruebaFiltroAnio(cont, informes) {
  cont.innerHTML = '';
  const presentes = ORDEN.filter((fam) =>
    informes.some((inf) => (inf.bloques || []).some((b) => { const f = familiaMA(b); return f && f.key === fam; })));
  if (!presentes.length) {
    cont.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: 'Sin pruebas tabulables en los informes cargados.' }));
    return 0;
  }
  const anios = [...new Set(informes.map((i) => i.ano).filter((a) => a != null))].sort((a, b) => a - b);
  const state = { fam: presentes[0], year: 'all' };

  const lbl = (txt) => Object.assign(document.createElement('div'),
    { className: 'pe-toolbar-lbl', textContent: txt });
  const tabbar = document.createElement('div'); tabbar.className = 'pe-tabs';
  ...
  const render = () => {
    body.innerHTML = '';
    const infs = state.year === 'all' ? informes : informes.filter((i) => i.ano === state.year);
    const sec = seccionFamilia(state.fam, infs);
    if (sec) body.appendChild(sec);
    else body.appendChild(Object.assign(document.createElement('p'),
      { className: 'muted', textContent: `${NOMBRE[state.fam]}: sin datos para ${state.year === 'all' ? 'los años cargados' : state.year}.` }));
    [...tabbar.children].forEach((b) => b.classList.toggle('is-active', b.dataset.fam === state.fam));
    [...yearbar.children].forEach((b) => b.classList.toggle('is-active', b.dataset.year === String(state.year)));
  };

  presentes.forEach((fam) => {
    const b = document.createElement('button');
    b.className = 'pe-tab'; b.dataset.fam = fam; b.textContent = NOMBRE[fam];
    b.onclick = () => { state.fam = fam; render(); };
    tabbar.appendChild(b);
  });
  const mkYear = (val, label) => {
    const b = document.createElement('button');
    b.className = 'pe-tab pe-tab-year'; b.dataset.year = String(val); b.textContent = label;
    b.onclick = () => { state.year = val; render(); };
    return b;
  };
  yearbar.appendChild(mkYear('all', 'Todos los años'));
  anios.forEach((y) => yearbar.appendChild(mkYear(y, String(y))));

  cont.appendChild(lbl('Prueba')); cont.appendChild(tabbar);
  cont.appendChild(lbl('Año')); cont.appendChild(yearbar);
...
export function montarPanelPrueba(cont, familia, informes) {
  ...
  h.style.cssText = 'margin:6px 0 2px;font-size:18px;color:#0f172a';
  cont.appendChild(h);

  const anios = [...new Set(informes
    .filter((inf) => (inf.bloques || []).some((b) => { const f = familiaMA(b); return f && f.key === familia; }))
    .map((i) => i.ano).filter((a) => a != null))].sort((a, b) => a - b);

  const lblY = Object.assign(document.createElement('div'), { className: 'pe-toolbar-lbl', textContent: 'Año' });
  const yearbar = document.createElement('div'); yearbar.className = 'pe-tabs';
  const body = document.createElement('div');
  const state = { year: 'all' };

  const render = () => {
    body.innerHTML = '';
    const infs = state.year === 'all' ? informes : informes.filter((i) => i.ano === state.year);
    const grupos = gruposDe(familia, infs);
    if (!grupos.size) {
      body.appendChild(Object.assign(document.createElement('p'),
        { className: 'muted', textContent: `Sin datos de ${NOMBRE[familia]} para ${state.year === 'all' ? 'los años cargados' : state.year}.` }));
    } else {
      const root = document.createElement('div'); root.className = 'pe-fus';
      [...grupos.entries()].sort((a, b) => a[0].localeCompare(b[0])).forEach(([g, entradas]) => {
        entradas.sort((a, b) => (a.inf.ano || 0) - (b.inf.ano || 0));
        if (state.year === 'all') {
          root.appendChild(cardResumen(familia, g, entradas));
        } else {
          root.appendChild(cardDetalle(familia, g, entradas));
        }
      });
