...
| **Estado al 2026-05-03**    | Refactor mix multi-modelo en curso (5 commits) · branch `claude/adjust-website-pages-8Ntwz`. **Commits 1-4 cerrados**: dominio puro + UI completa (selector + tabla mix con cantidad editable + banner APROBADO/NO + panel de sugerencias con 3 cards "Aplicar") + informe AFINIA refleja mix (sec 5 con N fichas técnicas, sec 8 con tabla mix + banner + sugerencias, sec 9 con grupos + breaker único, sec 10 BOM agrupado) + persistencia colección `acciones_refrigeracion` con rules + 4 índices + data layer + modal "Registrar acción de mantenimiento". Pendiente: commit 5 (tab "Consolidado Sistemas de Refrigeración" + tabla realtime + filtros + KPIs + export). ⚠ **Requiere deploy manual** del director (regla §0.1.1): `firebase deploy --only firestore:rules` + `firebase deploy --only firestore:indexes`. Mantenimiento Brigada base cerrado: 10 commits del módulo + fix `b4606cd` (PR #128) ya en `main`. v2.8.1 mergeada (PR #119). Versiones publicadas: **v2.5.0** → **v2.9.0**. CLAUDE.md ampliado con §0.1.2.1, §0.1.2.2, §0.1.2.3. 501/503 tests verdes (los 2 fallos son pre-existentes de importador Excel sin relación con este trabajo). Documentación: `docs/MANTENIMIENTO-BRIGADA.md` § 4.3 (dominio mix) + § 4.4 (UI mix) + § 4.5 (informe AFINIA con mix) + § 4.6 (persistencia). **Pendiente:** revocar PATs históricos. Cleanup de los 7 PDFs subidos por error al raíz del repo (commit `91f386c`). |
| Próxima movida              | Continuar el refactor mix multi-modelo: commit 2 (UI). Después commits 3-5 secuenciales. Director valida visualmente el informe AFINIA en producción al cierre del commit 3. Eventualmente ampliar el módulo "Mantenimiento Brigada" con nuevas calculadoras (aceite, aterramiento, etc.) sin tocar el sidebar. |
