...
| **Estado al 2026-05-03**    | Refactor mix multi-modelo en curso (5 commits) · branch `claude/adjust-website-pages-8Ntwz`. **Commit 1 cerrado**: dominio puro + 18 tests (62 totales en refrigeración). Pendientes: commit 2 (UI selector agregar-al-mix con cantidad + tabla con estado APROBADO/NO + sugerencias), commit 3 (informe AFINIA refleja mix · BOM agrupado por modelo · sec 5 con N fichas), commit 4 (persistencia `acciones_refrigeracion` + rules + indexes + botón "Registrar acción"), commit 5 (tab "Consolidado Sistemas de Refrigeración" + tabla realtime + filtros + KPIs + export). Mantenimiento Brigada base cerrado: 10 commits del módulo + fix `b4606cd` (PR #128) ya en `main`. v2.8.1 mergeada (PR #119). Versiones publicadas: **v2.5.0** → **v2.9.0**. CLAUDE.md ampliado con §0.1.2.1, §0.1.2.2, §0.1.2.3. 501/503 tests verdes (los 2 fallos son pre-existentes de importador Excel sin relación con este trabajo). Documentación: `docs/MANTENIMIENTO-BRIGADA.md` + `docs/UI-V3-DARKMODE.md` + `docs/MICROCIRUGIA-CONTRATOS-2026-04-27.md` + esta §9. **Pendiente:** revocar PATs históricos. Cleanup de los 7 PDFs subidos por error al raíz del repo (commit `91f386c`). |
