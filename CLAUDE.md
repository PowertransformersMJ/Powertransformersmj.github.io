...
| **Estado al 2026-05-03**    | Refactor mix multi-modelo **CERRADO** (5 commits, branch `claude/adjust-website-pages-8Ntwz`). Resumen: dominio puro (62 tests refrigeración) + UI completa (selector + tabla mix con cantidad editable + banner APROBADO/NO + panel de sugerencias) + informe AFINIA refleja mix (sec 5 con N fichas técnicas, sec 8 con tabla mix + banner + sugerencias, sec 9 con grupos + breaker único, sec 10 BOM agrupado) + persistencia colección `acciones_refrigeracion` con rules + 4 índices + data layer + modal "Registrar acción de mantenimiento" + tab "Consolidado Sistemas de Refrigeración" con tabla realtime + filtros + 5 KPIs + export CSV + acciones admin (cambiar estado / eliminar). ⚠ **Requiere deploy manual** del director (regla §0.1.1): `firebase deploy --only firestore:rules` + `firebase deploy --only firestore:indexes`. Sin ellos el botón "Registrar acción" y la tab Consolidado fallarán con `permission-denied` / `FAILED_PRECONDITION`. Mantenimiento Brigada base cerrado: 10 commits del módulo + fix `b4606cd` (PR #128) ya en `main`. v2.8.1 mergeada (PR #119). Versiones publicadas: **v2.5.0** → **v2.9.0**. CLAUDE.md ampliado con §0.1.2.1, §0.1.2.2, §0.1.2.3. 501/503 tests verdes (los 2 fallos son pre-existentes de importador Excel sin relación con este trabajo). Documentación: `docs/MANTENIMIENTO-BRIGADA.md` § 4.3 (dominio mix) + § 4.4 (UI mix) + § 4.5 (informe AFINIA con mix) + § 4.6 (persistencia) + § 4.7 (tab Consolidado). **Pendiente:** revocar PATs históricos. Cleanup de los 7 PDFs subidos por error al raíz del repo (commit `91f386c`). |
| Próxima movida              | (1) Director ejecuta los 2 deploys Firebase para activar la persistencia y la tab Consolidado. (2) Director valida en producción el flujo completo: agregar mix multi-modelo → ver banner APROBADO/NO → aplicar sugerencias si aplica → exportar informe AFINIA con N fichas + BOM agrupado → registrar acción → verla en la tab Consolidado. (3) Mergear branch `claude/adjust-website-pages-8Ntwz` a `main` cuando la validación pase. (4) Eventualmente ampliar el módulo "Mantenimiento Brigada" con nuevas calculadoras (aceite, aterramiento, etc.) sin tocar el sidebar. |
