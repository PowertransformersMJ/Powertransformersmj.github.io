...
| **CF-04** | ⚠️ *atenuado por `§83`* (el diagrama se guarda y vuelve a SU equipo); queda la limpieza al cambiar de datos: **los diagramas sobreviven al cambio de datos** y pueden reaparecer dibujados sobre otro equipo: `olvidarDiagramas()` solo corre al desmontar el tablero. | `panel.js:2873-2877` vs `:2815` | Llamar `olvidarDiagramas()` dentro de `fijarDatos`, donde ya se limpia el resto. **Una línea** + prueba |
