...
|---|---|
| **Misión ahora** | **ADR-057 (07-24→28): importador del Excel real LISTO y MO.00418 Ed.02 RATIFICADO tabla por tabla** — eval_dga=promedio(4), CRG=% oficial Planificación AT, HER=inspección 1-5; dry-run 208/213 con coincidencias ≥93-100%. **El parque enciende con UN paso del Ingeniero** (drag&drop Simulación→Importar en su Chrome). ⏭️ El Ingeniero anunció NUEVAS INSTRUCCIONES (próxima sesión). Restante autónomo: TODO-17 (menor) · template sanitizado + TPT/Respaldo fila-cabecera (tras import). Del Ingeniero: import · capítulo pruebas eléctricas del MO (TODO-04) · TODO-05/06/08 · GitHub Support · decisiones G017/G111/STRUCT. Detalle → `10` + `99 §57`. |
...
| **Deuda crítica** | 🔴 Del Ingeniero: (a) **drag&drop del import** (paso único, `10`); (b) **GitHub Support** purga `refs/pull/*` + revocar PATs (TODO-08); (c) **capítulo pruebas eléctricas del MO** (per-clase; el anexo Salud ya entregado y ratificado NO las trae, ADR-057). (✅ billing resuelto · ✅ B/E + extracción IA validadas en vivo ADR-054/bitácora 07-23 — ítem viejo corregido 07-28 · ✅ alerta GCP $5/mes.) |
