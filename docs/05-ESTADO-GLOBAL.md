...
|---|---|
| **Misión ahora** | **`99 §80` (09-20)**: manda el Excel en todos los caminos — la nube dejó de borrar la condición al registrar una muestra (**L-96**, cierra TODO-52/59). Antes `§78` (cédulas) y `§79` (rol desde el perfil). |
| **Build** | 🟢 **1706 pass / 0 fail / 2 skip** + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado; Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-16 · CI y Deploy en VERDE, comprobado en el log (L-65) |
