...
| Señal | Valor |
|---|---|
| **Build** | 🟢 `npm test` (lint + `node --test tests/*`) 1012/1012 verde (2026-06-07, hasta biblioteca-hub + fix desviación relación). |
...
| **Producción** | `main` → GitHub Pages (`pages.yml`). `origin/main` = `2bc0ca3` (lotes 4–5 en prod); `origin/DESARROLLO` = `1f51b70`; **HEAD local = `7707f65`** (lotes 6–8 + workflow + Tendencia F1 + Biblioteca-hub + fix desviación relación, sin pushear). **Pendiente push (director)**: lotes 6–8 + workflow + pestaña Tendencia + **Biblioteca como hub** (fusiona Informes cargados + accesos PDF). Solo lote 7 + workflow tocaron función (ya desplegados); el resto es frontend. |
