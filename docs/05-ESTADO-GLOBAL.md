...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). `99 §83` (09-22): lo redactado ya no se pierde (borrador local). Antes: `§82` cada TX abre SU ficha · `§81` potencia en la casilla · `§80` manda el Excel. |
| **Build** | 🟢 **1753 pass / 0 fail / 2 skip** + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-20 · CI y Deploy en VERDE, comprobado en el log (L-65) |
