...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). `99 §87` (09-23): **Tanda B en RAMA, sin publicar** hasta su visto bueno al preview. Antes: `§85` el alcance del formato · `§84` la carga tardía · `§83` el borrador · `§82` cada TX abre SU ficha. |
| **Build** | 🟢 **1798 pass / 0 fail / 2 skip** (en la rama; `main` sigue en 1759) + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-23 (local; CI y Deploy en VERDE = `main`, L-65) |
| **Branch / Deploy** | `DESARROLLO-/-PROYECTO-MJ` **por delante de `main` con la Tanda B** (código + cerebro, retenida hasta su visto bueno) · `main` == `origin/main` (SHA vivo → handoff hook o `git fetch`, nunca de memoria; se commitea+pushea+mergea en el mismo turno). **Historia reescrita 2026-07-21** (filter-repo purgó confidenciales) → otra copia debe re-clonar. |
