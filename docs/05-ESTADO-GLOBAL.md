...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes**. `99 §82` (09-20): cada TX abre SU ficha (el código de subestación no es la identidad). Antes: `§81` potencia en la casilla · `§80` manda el Excel · `§79` rol desde el perfil. |
| **Build** | 🟢 **1726 pass / 0 fail / 2 skip** + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-20 · CI y Deploy en VERDE, comprobado en el log (L-65) |
