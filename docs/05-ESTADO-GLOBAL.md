...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). `99 §85` (09-23): el alcance lo escribe la redacción del formato. Antes: `§84` la carga tardía · `§83` el borrador. Antes: `§82` cada TX abre SU ficha · `§81` potencia en la casilla · `§80` manda el Excel. |
| **Build** | 🟢 **1759 pass / 0 fail / 2 skip** + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-23 · CI y Deploy en VERDE, comprobado en el log (L-65) |
