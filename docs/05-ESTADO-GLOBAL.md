...
|---|---|
| **Misión ahora** | **`99 §75` (09-10) — FICHAS TÉCNICAS**, tanda larga: 5 afirmaciones falsas del papel firmado corregidas · el alcance ofrece las **7 macroactividades** y **argumenta** cada actividad con su sustento técnico · **define** las condiciones que el activo presenta (riesgo + afectación a clientes) · el **municipio** sale solo de la subestación. Lecciones **L-86/87/88**. Detalle en `99 §75.1-75.13`; abiertos en `10`. |
| **Build** | 🟢 **1643 pass / 0 fail / 2 skip** + `lint:html` limpio + **51 tests de reglas** (8 Firestore + 43 Storage; Storage necesita el emulador de Firestore al lado — L-78). · verificado-vivo: 2026-09-10 · CI y Deploy en VERDE, comprobado en el log (esperar SIEMPRE a que el Deploy termine — L-65) |
