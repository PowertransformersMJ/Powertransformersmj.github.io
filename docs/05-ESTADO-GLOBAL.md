...
| Señal | Valor |
|---|---|
| **Build** | 🟢 `npm test` (lint + `node --test tests/*`) **1096/1096 verde** (2026-06-08, +timeout INTERNO por intento de la IA: el cuelgue ya no deja el reproceso colgado, ADR-017). |
...
| **Deploys backend** | (1) `extraerPruebasElectricasIA` **DESPLEGADA** (southamerica-east1; canal `extra` por punto; prompt SIN columna "Evaluación/OK", L-42; **re-deploy 2026-06-08: (a) reintento con backoff de fallos transitorios — `functions/reintentos.mjs`, `maxRetries:0`, timeout 540→900s/1GiB, ADR-015/L-44; (b) REPROCESO server-side: persiste + estado durable `reproceso.{estado}`, modo `informeId`, ADR-016/L-45; (c) **timeout INTERNO por intento de la IA (`conTimeoutAbortable` aborta el stream colgado) + watchdog global + memoria 2GiB — el cuelgue ya NO deja el reproceso colgado en 'en_curso', ADR-017/L-46**). "Reprocesar" = trabajo asíncrono observable y con estado terminal GARANTIZADO. ⚠️ audit log de functions:log rezagado; 2GiB/900s desde fuente; validación en navegador la hace el director). (2) `narrativaTendenciaIA` **DESPLEGADA** (southamerica-east1; F3, narrativa sin PDF, default sonnet, 120s/512MiB). Secret `LLM_API_KEY` ok. Firestore con `experimentalAutoDetectLongPolling` (L-38). **Flujo (ADR-005)**: Claude commitea+deploya; el **director pushea**. |
