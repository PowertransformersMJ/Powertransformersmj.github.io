...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). **Publicado 09-23** (`f3411fb`): `99 §87` el Excel que se firma · `§88-§89` firmantes dictados · `§90` alcance sin acciones · `§91` fechas con calendario. Antes: `§85` el alcance del formato · `§84` la carga tardía · `§83` el borrador. |
| **Build** | 🟢 **1823 pass / 0 fail / 2 skip** + `lint:html` limpio + **93 tests de reglas** (50 Firestore + 43 Storage) + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-23 (local; CI y Deploy en VERDE = `main`, L-65) |
