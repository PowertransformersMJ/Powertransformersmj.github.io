...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). **Publicado 09-25** (`098dbff`, `0499e0d`, `01c921a`): `§93` Valor Real en blanco · `§94` calendario de años · `§95`-`§97` beneficios por práctica (breves, técnicos, sin marcas de fábrica) · `§98` firma estampada. **En rama**: `§99` firmas del equipo (espera su «procede» + deploy de reglas). |
| **Build** | 🟢 **1865 pass / 0 fail / 2 skip** + `lint:html` limpio + **107 tests de reglas** + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-25 (local; CI y Deploy en VERDE = `main`, L-65) |
