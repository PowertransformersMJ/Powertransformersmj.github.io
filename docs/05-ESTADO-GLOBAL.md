...
| **Producción** | `main` → GitHub Pages (`pages.yml`). `origin/main` = `2bc0ca3` (lotes 4–5 en prod); `origin/DESARROLLO` = `1f51b70`; **HEAD local = `bbc9691`** (lotes 6+7+workflow sin pushear). **Pendiente push (director)**: lote 6 (tabla determinista) + 7 (excitación) + **workflow de auditoría por sección** (`scripts/audit-bloques-pruebas.mjs` + auto-chequeo universal en prompt + hoja). |
