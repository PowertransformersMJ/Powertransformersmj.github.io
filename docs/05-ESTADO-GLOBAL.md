...
| **Producción** | `main` → GitHub Pages (`pages.yml`). `origin/main` = `2bc0ca3` (lotes 4–5 en prod); `origin/DESARROLLO` = `1f51b70`; **HEAD local = `2e577a4`** (lotes 6–8 + workflow + Tendencia F1 + Biblioteca-hub, sin pushear). **Pendiente push (director)**: lotes 6–8 + workflow + pestaña Tendencia + **Biblioteca como hub** (fusiona Informes cargados + accesos PDF). Solo lote 7 + workflow tocaron función (ya desplegados); el resto es frontend. |
