...
|---|---|
| **Misión ahora** | **ADR-063 (08-17): remediada la cola de la auditoría** — funciones desplegadas con topes, 53 índices Firestore vivos, 5 suscripciones acotadas, 13 escapadores completos, foto de fondo −78%. Antes: ADR-062 (auditoría holística), ADR-061 (Fichas Técnicas en la página). ⏭️ FALTA validar Fichas con **sesión real** (TODO-30). Pendientes → `10`. |
| **Build** | 🟢 `node --test` **1332 pass / 0 fail / 2 skip** (273 suites) + `lint:html` limpio + 8 tests de reglas (emulador, G025). CI corre los tests. · verificado-vivo: 2026-08-17 · CI en VERDE desde 08-16 (tras 76 corridas en rojo) |
