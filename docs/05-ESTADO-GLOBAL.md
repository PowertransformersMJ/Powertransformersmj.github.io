...
| Señal | Valor |
|---|---|
| **Build** | 🟢 `npm test` (lint + `node --test tests/*`) 1011/1011 verde (2026-06-07, lote 8 + Tendencia F1). |
...
| **Producción** | `main` → GitHub Pages (`pages.yml`). `origin/main` = `2bc0ca3` (lotes 4–5 en prod); `origin/DESARROLLO` = `1f51b70`; **HEAD local = `858e378`** (lotes 6–8 + workflow + Tendencia F1, sin pushear). **Pendiente push (director)**: lotes 6–8 + workflow auditoría + **pestaña Tendencia (Fase 1)**. Solo lote 7 + workflow tocaron función (ya desplegados); el resto es frontend. |
