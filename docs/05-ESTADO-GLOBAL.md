...
- **Flujo git (ADR-005)**: Claude commitea+deploya, el director pushea/mergea (runtime 403, L-01); nunca force-push a `main`. **Tablero + Tendencia F1-F3 consolidados** (ADR-009/010); **veredicto 100% NORMATIVO** (ADR-011, L-36): scorecard/KPI/matriz/tendencia/**badges por bloque** derivan del VALOR vs norma (no del texto del informe/IA); aislamiento NETA por clase + **resistencia ≤2% (NETA §7.2.2.D.8, vía skill)** unificados en el dominio; `conCriterios` sobrescribe el `limite_desbalance` de la IA. Criterios: tan δ/bujes ≤1% (IEEE 62), excitación Δ≤10%, relación ±0.5%, resistencia ≤2%, aislamiento NETA por clase (110 kV→30 GΩ). ⚠️ MO.00418 por clase sigue `verificar` (TODO-08). Próximo: validar más secciones con informes reales; TODO-01/02/08 abiertos.
