...
|---|---|
| **Misión ahora** | **FICHAS TÉCNICAS, por partes** (cola viva → `docs/cola-fichas-tecnicas.md`). **Publicado 09-25** (`098dbff`, `0499e0d`, `01c921a`): `§93` Valor Real en blanco · `§94` calendario de años · `§95`-`§97` beneficios por práctica (breves, técnicos, sin marcas de fábrica) · `§98` firma estampada · `§99` firmas del equipo (`42ba8bf`, `86d16a9`). ⚠️ Storage no entrega las imágenes (503, `§99.13`): no se estampa. |
| **Build** | 🟢 **1868 pass / 0 fail / 2 skip** + `lint:html` limpio + **108 tests de reglas** + `test:trigger` (1, emulador de Functions) (Storage necesita el emulador de Firestore al lado — L-78) + candado `guardia:cedulas`. · verificado-vivo: 2026-09-25 (local; CI y Deploy en VERDE = `main`, L-65) |
...
| **Backend** | Firebase `lordpowertransformersmj` (Auth + Firestore + Storage). **Billing REACTIVADO (2026-07-23)**. **4 CF desplegadas con `maxInstances`**: `extraerPruebasElectricasIA` · `narrativaTendenciaIA` · `onMuestraCreate` · `cronAlertasDiarias`. **53 índices declarados en el archivo**; la igualdad contra el servidor **sin verificar desde 2026-08-17** (se comprueba con `firestore:indexes`, no con el archivo — L-66). **Storage**: `firmas/{uid}` (`99 §71/§73`) y `firmas-equipo/` (`§99`, 2026-09-25). **Firestore**: `ordenes_materiales` (+lápidas) y `responsables_ordenes` desplegadas 2026-09-16 (`§77/§78`). |
