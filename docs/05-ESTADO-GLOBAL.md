...
|---|---|
| **Misión ahora** | **`99 §76` (09-16)**: Órdenes de Materiales al día con la versión del 8-sep, sin firmas ni cédulas y con el parque vivo; la validación cazó que recuperar una orden devolvía siempre la primera persona (**L-90**). Antes: **`§75`** — Fichas Técnicas (5 redacciones por banda, alcance argumentado, matriz a color, municipio automático). |
| **Build** | 🟢 **1654 pass / 0 fail / 2 skip** + `lint:html` limpio + **51 tests de reglas** (8 Firestore + 43 Storage; Storage necesita el emulador de Firestore al lado — L-78). · verificado-vivo: 2026-09-16 · CI y Deploy en VERDE, comprobado en el log (esperar SIEMPRE a que el Deploy termine — L-65) |
