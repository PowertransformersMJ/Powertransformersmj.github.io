...
|---|---|
| **Misión ahora** | **`99 §75` (09-10)**: cuatro lentes sobre **Fichas Técnicas**, cada hallazgo refutado por un escéptico ⇒ **36 confirmados**. Remediadas las **5 que hacían mentir al papel firmado** — entre ellas el dinero con centavos entrando **×100** al PE.02081 y el documento de mantenimiento proponiendo reposición **contra la orden del 09-09**. Luego (`§75.9-75.10`) el alcance ofrece **las 7 macroactividades** y lo referencial deja de contratarse entero (**L-87**), y el municipio sale solo de la subestación (`§75.11`) y el alcance **argumenta** cada actividad en vez de enumerarla (`§75.12`, **L-88**). Las 31 restantes → `10` TODO-35/58. Antes: `§74.23-74.24` (cargabilidad). |
| **Build** | 🟢 **1632 pass / 0 fail / 2 skip** + `lint:html` limpio + **51 tests de reglas** (8 Firestore + 43 Storage; Storage necesita el emulador de Firestore al lado — L-78). · verificado-vivo: 2026-09-10 · CI y Deploy en VERDE, comprobado en el log (esperar SIEMPRE a que el Deploy termine — L-65) |
