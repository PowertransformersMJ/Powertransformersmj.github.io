...
| **Producción** | `main` → GitHub Pages (`pages.yml`). `origin/main` = `2bc0ca3` (lotes 4–5 en prod); `origin/DESARROLLO` = `1f51b70`; **HEAD local = `df8cb9f`** (lotes 6+7 sin pushear). **Pendiente push (director)**: lotes 6 (tabla determinista) + 7 (excitación: Potencia, desviación ÷mayor, fix pdf.js). |
