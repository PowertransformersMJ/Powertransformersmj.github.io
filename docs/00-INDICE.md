...
| §46 | ADR-046 — Excitación: **orden por nivel (barras → curvas)** + **retira tablas REPETIDAS** del panel de gráficas (pestaña "Tabla de valores"=FUSIÓN + toggle "Magnitud") → la tabla desplegable ÚNICA (con veredicto) vive en el acordeón "Valores por prueba" (panel B). Pedido del director: "estas tablas sí, las demás elimínalas, es info repetida". NUEVO `_dev/preview-excitacion-fiel.html` (módulos REALES en `.pe-scope`+composición del shell). Código muerto FUSIÓN pendiente de poda. 1185/1185 (L-56) | 894 |
| §47 | ADR-047 — "Valores por prueba": un nivel de **MODO MIXTO** ('fases'+'items') tumbaba el panel → en producción solo se veía **AT·110** (faltaban AT·66/MT·34.5) aunque las gráficas sí los mostraban. Causa (L-55 p2): `cardResumen` aplicaba `filas[0].mode` a todas → `faseNames.forEach` sobre null → TypeError corta el bucle tras el 1.er acordeón. Fix: layout **por-fila** + guards + **try/catch por nivel**. REPRODUCIDO en navegador (antes 1 acordeón+throw, después todos). 1185/1185 | 912 |
| §48 | ADR-048 — Reorg del módulo POR PRUEBA (paso 1 de N, pedido del director "paso a paso, no generalices"): **"Corriente de excitación" = SEGMENTO unificado** (`.pe-seg`) que junta gráficas (`montarPanelExcitacion`) + tablas (`montarPanelPrueba`) en el orden actual + botón **"⬇ JSON de esta prueba"** (`descargarFichaPrueba`→`docs/pruebas/03-corriente-excitacion.json`). Excitación SALE de `PRUEBAS_VP` (no se duplica); se retira el `<h2>` redundante. CSS `.pe-seg-*` scoped. Demás pruebas INTACTAS (esperan su turno). 1185/1185 | 930 |
| §49 | ADR-049 — "**Nomenclatura y secciones de aislamiento**" (devanados H/X/Y + secciones CH/CL/CT/CHL/CLT/CHT) pasa **DENTRO del segmento Tan δ / Factor de Potencia — devanados** (pedido del director; era sección suelta). En `montarMultiAno` se reubica el nodo vivo `#nomencl` en `.pe-tand-seccion`; **rescate a `#tablero-scope` antes del `innerHTML=''`** para no destruirlo. `renderNomenclatura` intacto. 1185/1185 | 948 |
| §50 | ADR-050 — **Tan δ / FP = SEGMENTO unificado** (gráficas + tablas como excitación): el shell monta `montarPanelTand` + **`montarPanelPrueba('tand')`** + botón JSON, espejo de ADR-048. **Tablas vienen de `montarPanelPrueba`, NO del panel de gráficas** (L-57). "Todos los años" añade **detalle + evaluación POR INFORME** (todas las secciones, 0 huecos = tablas self-consistent; un `bloqueDiagnostico` por año). **Valores fuera de criterio en ROJO** (`claseCeldaTand`: `d-bad` no cumple >1% IEEE · `d-inv` investigar >0.5% NETA, solo cols tan δ + solo 'tand'). **Fix chips multi-norma**: `chipsCriterio` evalúa por NORMA vía `evaluarMultiNorma` (antes estado único → ✕ IEEE en 0.51% que cumple; L-58). Acotado a 'tand' salvo el fix de chips (corrección general). 1185/1185 · preview FIEL `_dev/preview-tand-tablas.html` | 968 |
