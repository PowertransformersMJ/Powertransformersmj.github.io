...
| **W-05** | **Testing de Cloud Functions (puro + integración)** | Antes de desplegar una CF (`southamerica-east1`) | Bugs de lógica + glue del trigger | Separar **lógica pura** (`node --test`, sin emulador) de **integración** (emulador real). Ojo: `functions/domain/` es **producto automático** del predeploy (`functions/prepare-deploy.mjs`, declarado en `firebase.json`): **nunca se edita** — se edita `assets/js/domain/` y el despliegue la regenera (`99 §68` · TODO-37). |
