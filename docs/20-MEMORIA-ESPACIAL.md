...
| **Órdenes de Materiales SSEE** (formato IT.05801, familia `.oms-`) | `pages/ordenes-materiales.html` + `assets/js/ordenes-materiales.js` + `assets/css/ordenes-materiales.css` + `assets/js/domain/ordenes_parque.js` (el desplegable Transformador lee el parque de Firestore tras la sesión, `99 §76`) + **registro del equipo** `assets/js/domain/ordenes_registro.js` (clave, lista blanca, qué se sube) + `assets/js/data/ordenes_materiales.js` (crear/editar/eliminar en transacción) → colecciones `ordenes_materiales/{TIPO_NUMERO}` y `ordenes_materiales_borradas/{clave}` (lápidas); en el navegador solo borrador, listas, `ssee.orden.pendientes.v1` y el histórico viejo congelado (`99 §77`). **Cédulas**: `domain/`+`data/responsables_ordenes.js` → `responsables_ordenes/{NOMBRE}`, leídas al generar el documento; candado `scripts/guardia-cedulas.mjs` + `githooks/*` con huellas en la bóveda (`99 §78`). **NO confundir con «Órdenes»** (`pages/ordenes.html`), que son las órdenes de TRABAJO y usan la colección `ordenes`. Historia → `99 §70` |
| **Firmas personales** (subir/ver/quitar la propia) | `assets/js/domain/firmas.js` (regla pura) + `assets/js/data/firmas.js` (Firestore, documento `firmas/{uid}` con la imagen; antes Storage, `§100`) + `assets/js/ui/firma-personal.js` + `assets/css/firma-personal.css`. Reglas en `storage.rules`. Se usa en Órdenes de Materiales, informe de refrigeración y **Fichas** (panel en `pages/fichas-tecnicas.html`; estampa `estamparFirmas` de `exportar-planificacion.js`; quién firma: `casillaEsDeLaSesion` en `fichas_firmantes.js`). Historia → `99 §71`, `§98`, `§100` |
| **Firmas del EQUIPO** (custodio admin; solo Fichas) | `assets/js/domain/firmas_equipo.js` (quién firma cada casilla) + `assets/js/data/firmas_equipo.js` (Firestore `firmas_equipo/{custodio}/personas/{persona}`, firma y registro en un lote; registros `firmas_equipo_registro` y `fichas_emisiones`) + `assets/js/ui/fichas/firmas-equipo.js` (panel) + botón «Descargar con firmas del equipo» en `panel.js`. Candado de git: `scripts/guardia-firmas.mjs`. Tamaño: `domain/firmas_tamano.js`. Vista previa del Excel + PDF: `ui/fichas/vista-previa-excel.js`. Historia → `99 §99`-`§102` |
