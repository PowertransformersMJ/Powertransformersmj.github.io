...
| **Firmas del EQUIPO** (custodio admin; solo Fichas) | `assets/js/domain/firmas_equipo.js` (quién firma cada casilla) + `assets/js/data/firmas_equipo.js` (Firestore `firmas_equipo/{custodio}/personas/{persona}`, firma y registro en un lote; registros `firmas_equipo_registro` y `fichas_emisiones`) + `assets/js/ui/fichas/firmas-equipo.js` (panel) + botón «Descargar con firmas del equipo» en `panel.js`. Candado de git: `scripts/guardia-firmas.mjs`. Historia → `99 §99`, `§100` |
