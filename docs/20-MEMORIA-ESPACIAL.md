...
| **Firmas personales** (subir/ver/quitar la propia) | `assets/js/domain/firmas.js` (regla pura) + `assets/js/data/firmas.js` (Storage, ruta `firmas/{uid}`) + `assets/js/ui/firma-personal.js` + `assets/css/firma-personal.css`. Reglas en `storage.rules`. Se usa en Órdenes de Materiales, informe de refrigeración y **Fichas** (panel en `pages/fichas-tecnicas.html`; estampa `estamparFirmas` de `exportar-planificacion.js`; quién firma: `casillaEsDeLaSesion` en `fichas_firmantes.js`). Historia → `99 §71`, `§98` |
| **Firmas del EQUIPO** (custodio admin; solo Fichas) | `assets/js/domain/firmas_equipo.js` (quién firma cada casilla) + `assets/js/data/firmas_equipo.js` (Storage `firmas-equipo/{custodio}/{persona}`; registros `firmas_equipo_registro` y `fichas_emisiones`) + `assets/js/ui/fichas/firmas-equipo.js` (panel) + botón «Descargar con firmas del equipo» en `panel.js`. Candado de git: `scripts/guardia-firmas.mjs`. Historia → `99 §99` |
