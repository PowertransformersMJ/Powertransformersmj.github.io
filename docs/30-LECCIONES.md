...
Síntoma: el director vio "OK" en la columna Evaluación de las tablas de detalle (tan δ, bujes, aislamiento) — ese "OK" lo emitía la IA (el prompt PEDÍA una columna "Evaluación"), violando "todo conforme a las normas" (L-36). Causa raíz: doble origen del veredicto por fila — (a) el prompt listaba "Evaluación" como columna a extraer en las barras → la IA ponía "OK"; (b) `derivarTablaTAP` añadía una columna "Eval." derivada ("OK"/"verificar"). **Receta (3 capas)**: (1) prompt: quitar "Evaluación/Calificación/OK" de las columnas (la IA emite SOLO datos crudos) + re-deploy de `extraerPruebasElectricasIA`; (2) `derivarTablaTAP`: dejar de añadir la columna "Eval." (mantener "Desv. %", que es un DATO); (3) render `tablaBloque`: STRIP defensivo de cualquier columna `/evaluaci|calificaci|veredicto|^eval\.?$/i` de las tablas de la IA → arregla los informes YA guardados sin re-extraer. El veredicto vive ÚNICAMENTE en el panel "Evaluación multi-norma" (valor vs cada norma + consolidado + recomendación). **Cómo evitarlo**: tablas = DATOS; veredicto = panel normativo. Si el prompt o el informe traen una columna de calificación, descártala en el render; nunca la muestres como verdad. Un punto sospechoso se marca con `verificar` (raya en la gráfica), no con un "OK"/"verificar" en una columna de tabla.
