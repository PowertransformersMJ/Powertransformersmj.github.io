...
**13.7 Doctrina + evolución** — "Cada prueba se evalúa Y se diagnostica conforme a la skill; donde no hay veredicto definitivo, se deja una recomendación — y la tendencia (degradación vs baseline) pesa tanto como el valor." Bujes discriminado en todo el flujo. **Pendiente**: los 2 informes ya cargados (2021/2023) necesitan re-carga para poblar el `bushing` canónico en la matriz/tendencia (el detalle ya se ve en el Tablero). Sin cache bump.
