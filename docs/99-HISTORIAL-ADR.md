...
**41.7 Archivos** — NUEVOS: `assets/js/ui/pruebas/excitacion-panel.js`, `tests/excitacion_panel.test.js`, `_dev/preview-excitacion.html`. MODIFICADO: `assets/css/pruebas-electricas.css` (+`.pe-tabla*`). INTACTOS: motor multi-norma, scorecard, `grafico-generico.js`, `tand-panel.js`, backend. **Doctrina**: "ir más allá" (memoria `feedback_ir_mas_alla`) — nivel por grupo vectorial, W de pérdidas faithful a la tabla del informe, criterio comparativo conforme a norma con su porqué visible. Cache n/a (frontend, sin SW). Falta: validar en la APP + cerrar pendiente de wiring al shell.
