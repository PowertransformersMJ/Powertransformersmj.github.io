...
**12.6 Archivos** — `domain/pruebas_electricas_multinorma.js` (nuevo: motor), `ui/pruebas/semaforo.js` (`calificarPrueba` delega), `ui/pruebas/grafico-generico.js` (`metricaBloque`/`multiNormaBloque`/`panelMultiNorma`/`badgeBloque`), `assets/css/pruebas-electricas.css` (`.pe-multinorma`/`.pe-mn-*`/`.mn-*`), `tests/*` (multinorma nuevo + data/semaforo migrados).
