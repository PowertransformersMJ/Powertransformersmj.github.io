...
**52.14 DECISIÓN FUERTE — CONECTAR FASE D (RBAC F28): NO activar ahora (2026-07-22, proceso-decision-fuerte + comité ×4)** — Corrí el pipeline de decisión fuerte (§G.2 🛰️) sobre "¿ampliar la escritura de `/ordenes` de admin-only a los 5 roles operativos?". Comité acotado de 4 (arquitecto-seguridad · escéptico · ejecutor · dominio-negocio), inline+schema sin tools (L-50). Crudo → bóveda `2026-07-22-decision-rbac-f28/comite-rbac-f28.json`. **VEREDICTO (adopto el unánime): NO activar ahora; mantener `/ordenes` admin-only en reglas + el feedback cliente de FASE C como defensa-en-profundidad genuina (NO teatro — es la postura más hermética con scaffolding listo).** Riesgos que cazó el comité y verifiqué contra el código: (1) **afloja, no refuerza** — hoy admin-only es lo más hermético; ampliar habilita a CERO usuarios (no existe UI no-admin: admin/ordenes.html es admin-guarded, pages/ordenes.html solo-lectura); (2) **fail-OPEN**: fallbacks (docs legacy sin `estado_v2`, ediciones sin cambio de estado) son superficies de bypass → escalada de privilegio invisible; (3) **deny-by-throw**: acceder a `resource.data.estado_v2` en docs legacy sin ese campo LANZA → rompe la evaluación de TODA la regla → bloquea al ADMIN → caída de producción; (4) **drift permanente** matriz `workflow.js`↔rules (rules no importa JS); (5) **scope-creep**: solo update/transición mapea a PERMISOS_TRANSICION — create/delete deben seguir admin-only (delete es destructivo); (6) **costo free-tier**: `profile()` = un get() por cada update. **Condiciones para el día D** (cuando el negocio lo exija): confirmar personas reales en los roles · construir la UI no-admin PRIMERO/junto · test de paridad emulador↔workflow.js + denegaciones + fallbacks fail-closed verde en CI · retener `isAdmin()` siempre-pasa · solo update/transición · rollout reversible en una línea co-entregando reglas+UI. **Secuencia correcta: consumidor→test verde→reglas→UI en el mismo rollout; NUNCA reglas ampliadas en el vacío.** **CONTRASTE (el Ingeniero no tiene Gemini → red-team interno de 3 críticos independientes sobre el problema CRUDO, sin ver la conclusión del comité; crudo `redteam-rbac-f28.json`): CONVERGE con el comité — esperar (2/3 explícitos; el "steelman activar-ahora" acepta NO abrir la escritura, solo preparar).** 3 HALLAZGOS NUEVOS que afilan la decisión y son PRERREQUISITOS del día D: (1) **`allow update` es AGNÓSTICO DE CAMPO** — la matriz PERMISOS_TRANSICION solo gobierna el campo `estado`; pero Firestore autoriza el doc ENTERO. Un rol con una transición válida podría mutar `costo`/`firmas`/`asignado_a` en la misma escritura, y una edición que NO cambia estado no es transición → un helper ingenuo la deja pasar. Implementar solo "la matriz" en reglas = FALSA completitud (el control más peligroso: parece total, no lo es). El enforcement correcto exige **constraints de inmutabilidad a nivel de CAMPO** (qué campos toca cada rol en cada estado), dimensión AUSENTE de workflow.js → depende de la UI/flujo, no se puede diseñar hoy. (2) **Custom claims**: leer el rol vía `get(/usuarios)` se factura en CADA escritura; el prerrequisito barato es poner el rol en `request.auth.token.role` (claims) → regla con CERO get(). Sin claims activar es caro. (3) **Brecha percibida-vs-real**: el dueño ve feedback de rol en cliente y podría CREER que los 7 roles están enforced; la verdad es que SOLO admin lo está (los otros 6 ni pueden escribir). **DECISIÓN FINAL (Claude, delegada por el Ingeniero 2026-07-22): NO activar D ni construir el prototipo de reglas ahora** (sería incompleto sin las constraints de campo, que dependen de la UI inexistente — over-engineering del futuro equivocado). Revisado con red-team interno (NO Gemini). Cache: n/a.

## 53. ADR-053 — "HAS TODO TU": G010 cableado (umbrales→HI) + validación normativa TODO-04 + endurecimiento FASE B/E

> Pedido del director (2026-07-23): *"HAS TODO TU"* — delegación total de lo ejecutable sin él. Regla operativa nueva del mismo día: **workflows acotados y en Opus; la cuota de Fable se reserva para análisis/decisión.**

**53.1 Causa raíz**: (a) **G010** (Fase 9, alta, re-verificado): la pantalla F18 `admin/umbrales-salud.html` persistía umbrales en `/umbrales_salud/global` con auditoría e historial, pero NINGÚN cálculo los consumía — el motor (`salud_activos.js`) usaba literales congelados y la CF `onMuestraCreate` tampoco leía la config: editar la pantalla no recalibraba nada (UI que miente). (b) **TODO-04**: 12 clusters de umbrales del lóbulo `49` marcados `⚠️ verificar` sin fuente confirmada; dos etiquetas de norma resultaron mal atribuidas. (c) **FASE E**: la tarjeta del evaluador ocultaba la causa real ("sin límite práctico" con respaldo sin ampacidad) y la tabla RCA de fallados interpolaba texto libre sin escapar (XSS almacenado, clase G024).

**53.2 Solución estructural**: (a) parámetro opcional de config en cada `calcularCalif*`, `evaluarDGA/ADFQ`, `aplicarOverrides` y `snapshotSaludCompleto(ctx.umbrales)` con **fallback bit a bit al baseline** (`_u()` merge defensivo por sección; claves `min→>=`, `min_excl→>`, `max→<=`, `max_excl→<`); cableados los 6 consumidores — `muestras.crear/actualizar`, importador Excel (`procesarLibro`+`admin/importar.html`), propuestas FUR (`abrirPropuestaFUR`), `motor-salud.html`, `demo-seed.html` y la CF (`leerUmbralesActivos` fail-safe, Admin SDK); el snapshot registra `umbrales_version` (trazabilidad). (b) workflow Opus acotado (4 clusters investigación + refutación por AJUSTE): 2 ajustes CONFIRMADOS y aplicados (re-atribución tan δ: NETA 100.3 aceite mineral=1.0%, el 0.5% es IEEE C57.12.90/C57.152 unidad nueva — etiquetas intercambiadas SIN tocar bandas; descriptor per-clase ya no cita NETA 100.5), 2 REFUTADOS (fallback 1 GΩ correcto; FP-vs-placa inviable sin dato de placa). Detalle y confirmados → `49 §Validación TODO-04`. (c) tarjeta E muestra `ev.recomendacion` y distingue Infinity/ausente; tabla RCA escapada con `escR`.

**53.3 No-regresión**: firmas ADITIVAS (cero renombres/renuncias de export); sin config el resultado es idéntico a los literales históricos (los 1187 tests previos son el guard y pasan sin tocar); bandas operativas de pruebas eléctricas INTACTAS (solo etiquetas); `PESOS_HI` (Tabla 10) intocado.

**53.4 Tests/verificación**: +7 tests G010 (bandas custom, herencia de config parcial, threading al HI, `hi_min` custom) → **1196 total · 1194 pass · 0 fail**; lint HTML OK; **revisión adversarial independiente del diff** (agente Opus): APROBADO, 0 defectos, operadores banda a banda verificados; **verificación E2E headless de FASE B/E** (agente Opus): ambas FUNCIONALES (B: estado-cero limpio, guardado válido, anti doble-submit; E: paths de schema v2 correctos, división-por-cero cubierta, XSS escapado).

**53.5 Anti-patterns evitados**: no fabricar datos (config ausente → baseline oficial MO.00418); sin `onSnapshot` nuevos (un `getDoc` por guardado, free-tier); CF fail-safe (el flujo de la muestra jamás se cae por la config); no se plegó el piso 0.5% a `calificarTanDelta` (habría movido investigar de >0.5 a >0.7 — regresión advertida por el refutador).
...
## 57. ADR-057 — Importador del Excel real "Salud de Activos" + MO.00418 Ed.02 ratificado (DGA/CRG/HER)

> Sesiones 2026-07-24 → 2026-07-28 (Fable 5 + Ingeniero). El Ingeniero entregó su Excel real (hoja TX_Potencia, 213 filas) y el MO.00418.DE-GAC-AX.01 Ed.02 (14/10/2025, 19 pp — confidencial, NUNCA al repo). Las 3 decisiones normativas que bloqueaban la importación quedaron resueltas.

**57.1 Causa raíz**: (a) el importador (F17) no entendía las cabeceras REALES del Excel del parque ("POTENCIA (KVA)", "AÑO DE FABRICACION", dobles espacios) → el HI se recalculaba solo con DGA+FUR (EDAD pesa 30%) e inflaba discrepancias; (b) 5 filas vacías al final pasaban a persistencia y habrían creado docs basura `UNK-*`; (c) la agregación DGA del motor era MAX(TDGC,C2H2), que deja CO/CO₂ fuera de la variable; (d) HER inaplicable: UBICACIÓN FUGAS es texto libre y el motor solo aceptaba el catálogo canónico; (e) el % de CARGA de las columnas de corriente contradecía a la columna CARGABILIDAD en 61 filas (fuente en disputa).

**57.2 Solución**: (a) alias ADITIVOS de cabeceras reales + colapso de espacios en `g()` (`49e21fb`); (b) guard `omitidos` en `persistirImportacion` (filas con `errores_validacion` NO persisten; contador en retorno/job/audit/UI); (c) **decisión CRG (Ingeniero 2026-07-27)**: CARGABILIDAD = dato OFICIAL de Planificación Alta Tensión → `calcularCalifCRG` acepta `crg_pct` directo con prioridad, cociente de respaldo (`af5a31d`); (d) **MO.00418 Ed.02 ratificado TABLA POR TABLA**: las 9 tablas + Tabla 10 (pesos) + 3 overrides = IDÉNTICOS al baseline ✓; **eval_dga = PROMEDIO REDONDEADO** de TDGC/CO/CO₂/C₂H₂ disponibles (nueva `calcularEvalDGA`, 3 callsites: snapshot, evaluarDGA, importador) — la norma califica los 4 grupos y "acumula el riesgo", práctica del área 201/206; **HER acepta calif 1-5 numérica** de inspección (Tabla 9 es escala cualitativa) leída de EVALUACION HERMETICIDAD, texto de fugas queda informativo (`f17af07`).

**57.3 No-regresión**: TODO aditivo — ningún alias/export renombrado; sin `crg_pct` el cociente sigue; sin numérico HER usa el catálogo; el guard solo EXCLUYE filas inválidas (antes basura). Import idempotente por `codigo`=MATRICULA verificado (actualiza los 206, no duplica).

**57.4 Verificación**: dry-run Node LOCAL contra el Excel real (scratchpad, sin persistir): 213 filas → 208 completas; coincidencia motor↔Excel factor a factor: TDGC 206/206 · CO 205 · CO₂ 206 · C₂H₂ 204 · IC 205 · FUR 206 · eval_dga **201/206 (antes 25)** · CRG **192/206 (antes 148)** · EDAD 192 · RD 188. Sondas de caracterización: CONDICION Excel = trunc(HI ponderado) en 150/206 + **juicio experto ~38 filas** (la norma lo reserva al Profesional — el motor NO lo imita; quedará como discrepancia esperada del reporte de import). EDAD ±1 (15 filas) = fecha de corte del Excel vs hoy (motor correcto). Suite: **1211 tests · 1209 pass · 0 fail** (3 tests reescritos a la agregación Ed.02 + 4 nuevos).

**57.5 Anti-patterns evitados**: no fabricar clasificación de fugas desde texto libre (§3.2) · la CONDICION del Excel NO se confía (recalcular, §D1) · sin jerga al Ingeniero · el PDF y el Excel jamás tocan el repo público.

**57.6 Archivos**: `domain/importador.js` · `domain/salud_activos.js` · `domain/umbrales_salud_baseline.js` (string `aggregacion`) · `data/importar.js` · `admin/importar.html` · `tests/importador.test.js` · `tests/salud_activos.test.js`. Commits `49e21fb`·`af5a31d`·`f17af07` + merges a `main` (`ae58a30`·`4da21da`·`c3c1c9b`). Dry-run scripts en scratchpad de sesión (efímeros, con rutas locales).
...
**58.1 Causa raíz** (verificada leyendo el ecosistema de origen y el propio, no supuesta): el repo y la bóveda vivían SUELTOS en el Escritorio y el kernel estaba **copiado dentro del repo** — brain-kit v1.0, sin `VERSION`, sin sello, y **ya forkeado localmente** (parche de 11 líneas en `brain-check.mjs` para leer `31-LECCIONES-IA`). Consecuencia estructural: toda mejora del cerebro moría en este repo y no había forma de que un proyecto nuevo naciera conectado. El delta real con el ecosistema de origen **no era documentación ni skills** (en hooks y `pre-commit` este repo iba por DELANTE de los suyos): era el **kernel canónico central con reparto sellado**. Deliberación: workflow de 10 agentes Opus (5 lentes + 5 verificadores adversariales), crudos + `SINTESIS.md` en `research-archive/2026-07-28-adr058-ecosistema/`.

**58.2 Solución estructural**: (a) **paraguas `~/Desktop/GitHub-MJ/`** con repo y `brain-private/` HERMANOS (la hermandad sostiene `archiveDir` y el reparto del kernel), más las carpetas de trabajo OLTC y `_archivo/`; (b) **kernel canónico PROPIO** en `brain-private/kernel/` (v1.7.0, `README.md` con linaje y delta), repartido con `npm run brain:pull` → `scripts/.kernel-version.json` sellado con SHA-256 y validado BLOQUEANTE por el gate #0; (c) **delta propio v1.7.0**: gate #5 acepta lecciones shardeadas (`30` + `3[1-9]-LECCIONES*.md`), siguiendo el idioma que el kernel ya usaba para el índice shardeado — arregla el KERNEL en vez de mutilar el contenido; (d) `docs/60-WORKFLOWS.md` (W-01..W-13, W-11 = SSoT del flujo fuerte; **W-12/W-13 son propios**); (e) `15-CONSEJO-EXTERNO` migrado a **Gemini vía Antigravity** con el error factual corregido; (f) `brain-private/NUEVO-PROYECTO.md` = la receta que responde literalmente al pedido.

**58.3 No-regresión**: cero cambios de producto — ni un archivo de `assets/`, `pages/`, `admin/`, `functions/`, `firestore.rules` ni `tests/`. `git mv` no aplicó: fue `mv` de directorios completos (rename atómico, misma partición), con `.git/config` de ambos repos verificado SIN rutas absolutas, sin submódulos ni worktrees. Ramas y remotos intactos (`DESARROLLO-/-PROYECTO-MJ`, `origin` sin divergencia). §2.2/§2.3 del nodo `15` (los 6 casos de decisión fuerte de SGM) **preservados textualmente**: la migración fue cirugía, no reescritura. La numeración propia `31-LECCIONES-IA` / `49` / `50-TRANSFORMADORES` se conservó pese a que el ecosistema de origen usa esos números para otra cosa.

**58.4 Tests/verificación**: `npm run test:unit` → **1211 tests / 1209 pass / 0 fail** (idéntico al baseline pre-migración) · `npm run lint` OK · `npm run brain:check` → `kernel v1.7.0 íntegro (6 archivos) == canónico v1.7.0` + `archiveDir íntegro` + `refs L-/M- (63/63) resuelven en 30+1 shard(s)` + `1 hecho SSoT sin duplicados` + `31 docs alcanzables` + **CEREBRO SANO** · `boot-gate` 30416c ≤ 31500c · `brain:diff` reconoce el paraguas. Rescate verificado: `~/.claude/projects/<slug-nuevo>/` con los 19 archivos de memoria y las 12 sesiones.

**58.5 Anti-patterns evitados (§3)**: **NO se fusionaron bóvedas** — la del ecosistema de origen tiene remoto en la cuenta GitHub de un TERCERO, y la nuestra guarda material real de cliente AFINIA/EPM: habría sido una fuga de datos de cliente hacia una cuenta ajena (§3.2 free-tier/confidencialidad, y el veto de `Debug/` y `450108/` extendido a la bóveda) · **NO se fusionó `31` en `30`** para contentar al linter, aunque cabía: eso destruía el sharding §G.5 — se arregló el kernel · **NO se rellenaron `ssotFacts` con MO.00418 ni el project id**: aparecen 20+ y 12+ veces como REFERENCIA legítima y habrían dado 30+ warns bloqueantes sin corregir riesgo alguno (green-tuning al revés; el descarte queda razonado en el manifest) · **NO se reescribió la historia** de `99 §51/§52/§55`: sus rutas quedan como registro de época y este ADR declara la vigente (§G.4 límite de guardián) · **NO se editó `~/.claude.json`** (47 KB, 5 proyectos) para ahorrar un diálogo de confianza: riesgo > beneficio · **NO se borró** el override stale de `comite-expertos`: cuarentenado en `_legacy/skills-stale-2026-07-28/`.

**58.6 Archivos modificados/INTACTOS**: MODIFICADOS — `CLAUDE.md` (§0 fila 60-WORKFLOWS + "57 ADRs"; §1/§7 raíz; §2 footer con modelo real; §G.2 triggers 🧪/🛰️; §G.4 bóveda-se-commitea, escalado acotado, backstop #6, kernel-no-se-edita-aquí), `docs/{00-INDICE,10,20-ESPACIAL,15-CONSEJO-EXTERNO,.brain-manifest.json}`, `package.json`, `.gitignore`, los 6 `scripts/*.mjs` del kernel (por `brain:pull`, no a mano). NUEVOS — `docs/60-WORKFLOWS.md`, `scripts/.kernel-version.json`, `scripts/brain-archive.mjs`, `brain-private/kernel/*` (8 archivos + README), `brain-private/NUEVO-PROYECTO.md`, `research-archive/2026-07-28-adr058-ecosistema/`. INTACTOS verificados — todo `assets/`, `pages/`, `admin/`, `api/`, `functions/`, `tests/`, `firestore.rules`, `firestore.indexes.json`, `storage.rules`, `docs/49`, `docs/50`, `docs/30`, `docs/31`, y las 15 hojas del dueño.

**58.7 Doctrina aplicada + deuda declarada**: §3.3 en todo (cada afirmación del plan se verificó en disco; los verificadores refutaron 6 claims de sus propios compañeros — ver `SINTESIS.md`) · §3.4 IAP · §G.4 Frescura, Neurogénesis, Captura (crudo + síntesis en la bóveda ANTES de cerrar) y Límite de guardián · §G.5 one-in-one-out (se retiró texto redundante para pagar las reglas nuevas). Cache §4: n/a (sección dormida). **DEUDA ABIERTA, declarada y no disimulada**: (1) 🔴 **la bóveda sigue SIN REMOTO** — un solo disco, con material de cliente; `lastOffsiteBackup` se dejó VACÍO a propósito para que el banner "EN CRISTIANO" siga avisando "Copia de seguridad externa: NUNCA hecha"; (2) el menú de modelos de `15 §0b` está portado del ecosistema de origen y **NO verificado en esta Mac** — confirmarlo al primer uso; (3) el heartbeat reporta **costo-cerebro 30d = 53% 🔴** contra bandera del 30%: el mantenimiento del cerebro consume demasiado del mes, y esta sesión lo empeora — la señal correcta es recortar doctrina, no añadirla; (4) `functions/domain/` diverge de `assets/js/domain/` en 3 archivos (hallazgo colateral, sin tocar).

## 59. ADR — Decisiones del Ingeniero: bóveda local, menú de Antigravity confirmado, autoridad de merge reafirmada ⟦OPUS-5⟧ (2026-07-29)

> Cierre del ADR-058 con las 3 decisiones que el Ingeniero tomó sobre la deuda que quedó abierta.

**59.1 Causa raíz**: ADR-058 cerró con 3 puntos que NO eran míos de decidir (respaldo de la bóveda, verificación del menú de Antigravity, y el merge a producción que retuve). El Ingeniero los resolvió los tres de una vez. El tercero, además, expuso un defecto MÍO de operación: pedí una autorización que su §2 ya me había concedido desde la entrevista F3a.

**59.2 Solución estructural**: (a) **BÓVEDA DE USO LOCAL** — sin repo remoto, por decisión explícita. Para que eso no dejara una alarma perpetua en el banner del arranque, el kernel sube a **v1.8.0**: `lastOffsiteBackup` pasa de 2 estados a 3, con el sentinel `"NINGUNA"` = *el dueño decidió que no hay copia externa*. El banner ahora dice el HECHO (*"ninguna, por decisión del dueño — la bóveda vive en UNA sola copia"*) en vez de pedir una acción descartada. (b) **`15 §0b` con nombres reales de modelo** (Gemini 3.1 Pro High = default · 3.6 Flash High = barridos · tiers bajos 🚫 · Claude ⛔ misma familia · GPT-OSS 120B = desempate), confirmado por el Ingeniero: su menú es el mismo del ecosistema de origen. Sigue marcado como hecho caducable L-30. (c) **Merge a `main` ejecutado** y lección **L-63** escrita. (d) Colateral necesario: al quedar la sesión anclada en el paraguas y no en el repo, nacieron `GitHub-MJ/CLAUDE.md` (router del ecosistema) y `GitHub-MJ/.claude/settings.json` (hook de `brain:diff`), y se replicó la memoria automática al proyecto paraguas.

**59.3 No-regresión**: cero cambios de producto. El cambio del kernel es aditivo y compatible hacia atrás: sin el sentinel, el comportamiento es idéntico al v1.7.1 (fecha → días transcurridos; vacío → "NUNCA hecha ⚠️"). El router del paraguas NO es una neurona y no toca el presupuesto de boot del repo: vive fuera de él.

**59.4 Tests/verificación**: `kernel v1.8.0 íntegro == canónico v1.8.0` tras `brain:pull` · banner verificado en vivo con la línea nueva · `brain:diff` desde el hook del paraguas OK · `brain:check` **CEREBRO SANO** · `boot-gate` dentro de objetivo · 1209 pass / 0 fail. **Verificado antes de escribir la decisión (§3.3)**: `tmutil destinationinfo` → *"No destinations configured"*, o sea que "solo local" hoy significa literalmente una sola copia — el riesgo está aceptado a sabiendas, no por desconocimiento.

**59.5 Anti-patterns evitados (§3)**: **NO se falsificó una fecha de respaldo** para acallar el aviso (habría sido una mentira en el cerebro, §3.3) — se cambió el kernel para poder decir la verdad · **NO se dejó la alarma perpetua**: un aviso que nadie va a atender degrada la credibilidad de TODOS los avisos · **NO se asumió** que "solo local" implicaba tener Time Machine: se comprobó · **NO se re-litigó** la decisión del Ingeniero tras plantear el riesgo una vez.
...
## 60. ADR — Hosting: revisión de la prohibición de uso comercial de GitHub Pages y descarte (por ahora) de Cloudflare Pages ⟦OPUS-5⟧ (2026-07-29)

> Pregunta llegada del proyecto hermano `mantenimiento-lineas-at` al verificar los ToS de Pages: ¿SGM · TRANSPOWER, **tal como está publicado hoy**, cae en la prohibición de uso comercial? **Decisión: NO se migra el hosting.** Este ADR existe para que la pregunta no se re-analice cada seis meses: trae el veredicto, el runbook de migración ya costeado, y los disparadores que SÍ obligan a reabrir.

**60.1 Causa raíz (qué prohíbe realmente el texto, verificado en `docs.github.com/en/pages/.../github-pages-limits` el 2026-07-29)**: la cláusula tiene **tres supuestos y una advertencia**, y hay que separarlos porque el proyecto no cae igual en todos. Supuestos: (1) *"free web-hosting service to run your online business"*; (2) *"e-commerce site"*; (3) *"any other website that is primarily directed at either facilitating commercial transactions or providing commercial SaaS"*. Advertencia aparte: *"GitHub Pages sites shouldn't be used for sensitive transactions like sending passwords or credit card numbers"*. Contraste contra el sitio real, leído archivo por archivo:
> · **(2) e-commerce → NO, y no marginalmente.** `grep -niE "factur|cobro|pago|tarifa|precio|cotiza|checkout|stripe|paypal|suscripci|comprar|venta"` sobre `index.html` + `home.html` + los 30 `pages/*.html` da **cero** coincidencias de dominio comercial. Los únicos montos en COP son **contabilidad interna de ejecución**: presupuesto comprometido/ejecutado/disponible del contrato macro (`admin/contratos.html:142-212`), valor unitario de suministros (`admin/suministros-catalogo.html:115`) y costo de reposición de un fallado (`admin/fallados.html:165`). Registran plata **ya gastada por el cliente**; no inician, cobran ni liquidan ninguna transacción. No hay pasarela, ni carrito, ni precio público.
> · **(3) transacciones comerciales / SaaS comercial → NO.** El sitio es **mono-inquilino y sin autoservicio**: no hay registro — `index.html:340-357` exige un perfil `/usuarios/{uid}` con `activo !== false` creado por un admin, y desloguea a quien no lo tenga. No se vende, no se licencia, no hay planes, no hay segundo cliente. Lo más cercano a "transacción" es el formulario de `pages/contacto.html` ("Solicitud de intervención"), y **no lo es**: está detrás del `page-guard`, va con `noindex,nofollow`, y produce un ticket interno trazable, sin precio, cotización ni pago.
> · **(1) "run your online business" → ZONA GRIS, y es la única real.** El pie del portal declara **LORD POWER TRANSFORMERS MJ S.A.S.** (`index.html:227`) y el proyecto Firebase es `lordpowertransformersmj`: hay una sociedad comercial detrás y el sitio es su columna operativa. Pero el negocio **no es el sitio**: el negocio es mantenimiento de transformadores en campo, y por el sitio no fluye ni un peso de ingreso. La lectura natural del supuesto —el sitio ES el negocio, como una tienda o un SaaS— no aplica; una lectura agresiva —herramienta de una empresa alojada gratis— sí rozaría. Riesgo **bajo, no nulo**.
> · **La advertencia SÍ se contradice literalmente**: `index.html:204` es un campo `type="password"`. Matiz honesto: es un *"shouldn't"*, no una prohibición; nació en la época en que Pages con dominio propio no forzaba HTTPS; y la credencial viaja por TLS a `identitytoolkit` de Google, no a servidores de GitHub. Es la frase que el sitio incumple al pie de la letra, y es también la más ignorada del ecosistema.

**60.2 Solución estructural (la decisión) — QUEDARSE en GitHub Pages, con la salida ya costeada**: el riesgo no se paga en dinero sino en **disponibilidad**: si GitHub decidiera aplicar la cláusula, la consecuencia no es una factura sino la **baja unilateral de la única URL de producción**, sin SLA ni plazo de apelación (tier gratuito = sin contrato de soporte). Frente a eso, migrar HOY cuesta más que el riesgo que evita: cambia la URL a todo el equipo, no elimina ningún riesgo verificado, y consume la sesión que `10` reclama para PRODUCTO (el parque real espera UN paso del Ingeniero). Se decide: **no migrar**, dejar el runbook escrito (60.4), y fijar disparadores explícitos (60.7). **Hallazgo colateral más grave que el consultado — Vercel Hobby**: sus términos son **mucho** más duros que los de Pages. Verificado en `vercel.com/docs/limits/fair-use-guidelines`: *"Hobby teams are restricted to non-commercial personal use only"*, y define uso comercial como *"any Deployment that is used for the purpose of financial gain of **anyone** involved in **any part of the production** of the project, including a paid employee or consultant writing the code"*. Esa definición sí atrapa a este proyecto sin zona gris. Y la superficie Vercel **no entrega nada**: `grep -rn "/api/"` sobre `assets/js/`, `pages/`, `home.html` y `admin/` da **cero** llamadas; `api/health.js` es solo una sonda de disponibilidad, y `vercel.json` duplica el sitio estático con headers y redirects. Recomendación al Ingeniero: **retirar el despliegue de Vercel** (o subirlo a Pro si algún día se usa) — quita el incumplimiento más claro de los tres proveedores y no cuesta una sola función del producto. Queda como decisión suya, no ejecutada aquí.

**60.3 No-regresión**: **cero líneas de producto tocadas**. No se modificó hosting, dominio, `pages.yml`, `vercel.json`, `firebase.json`, reglas ni funciones. El sitio sigue sirviéndose igual (verificado vivo en 60.4). Este ADR es análisis + registro; la única escritura fue al cerebro (`99`, `00`, `10`).

**60.4 Verificación + runbook de migración YA COSTEADO (Cloudflare Pages, backend Firebase intacto)**. Vivo hoy (`curl -o /dev/null -w "%{http_code}"`, 2026-07-29): `/` **200** · `/pages/inventario.html` **200** · `/docs/05-ESTADO-GLOBAL.md` **200** · `/CLAUDE.md` **200** — o sea, el repo entero se publica, incluido el cerebro (deliberado, F3a) **y el HTML de las páginas protegidas**: hoy la protección es guard en cliente + reglas de Firestore, así que el DATO está a salvo pero la ESTRUCTURA es pública. Encaje técnico verificado contra `developers.cloudflare.com`: plan Free = *"500 deploys per month"*, *"up to 20,000 files"*, *"maximum file size … 25 MiB"*, 100 dominios propios, 1 build concurrente → el repo tiene **923 archivos tracked / 13 MB / ningún archivo cerca de 25 MiB**: cabe con 20× de holgura. *"Both private and public repositories are supported"* en la integración Git → sirve desde repo PRIVADO sin costo (lo que Pages solo da con GitHub Pro, y aun pagando el sitio queda público). Cloudflare Access puede cubrir el hostname de producción `*.pages.dev` (Known Issues: *"in the **Subdomain** field, delete the wildcard"*), Zero Trust free hasta 50 usuarios → no exige comprar dominio. ⚠️ Un dato NO confirmado en la doc: el "ancho de banda ilimitado" no aparece escrito en la página de límites de Pages (que sí documenta archivos/builds/tamaño); GitHub, en cambio, documenta *"soft bandwidth limit of 100 GB per month"*. **Runbook (medio día, ~4-6 h, sin tocar el backend)**: Firebase, Firestore, Storage y las CF son agnósticas del host — las dos funciones son `onCall` (`functions/index.js:542,767`), que no dependen del origen. (1) proyecto en Cloudflare Pages contra el repo, sin build, output `/` (~15 min); (2) Firebase Console → Auth → **dominios autorizados** + `<proyecto>.pages.dev`, necesario para los enlaces de recuperación de contraseña (~2 min); (3) **gotcha verificado**: Cloudflare redirige `/x.html` → `/x` (*"`/contact.html` will be redirected to `/contact`"*) — los enlaces siguen sirviendo por el 308, pero `assets/js/aqua-shell.js:97` compara con `cleanPath.endsWith('/admin/usuarios.html')` y **dejaría de disparar** los redirects legacy→pestaña; fix aditivo normalizando la extensión antes de comparar (~30 min + test). Los otros chequeos de ruta (`aqua-shell.js:42`, `data/indicadores_calidad.js:30`, `data/seguimiento_cargabilidad.js:28`) comparan contra `/pages/` y `/admin/` (directorio) y **sobreviven**; (4) 7 URLs absolutas al dominio viejo: `index.html:13,20,23,29`, `home.html:12`, `package.json:7` y `functions/index.js:184` (enlace del correo de alertas → apuntaría a un host muerto) (~20 min); (5) `sitemap.xml` + línea `Sitemap:` de `robots.txt` (~5 min); (6) los dos hosts pueden convivir durante la transición (gratis, y es el camino seguro); (7) verificación end-to-end §G.4 Caza-bugs: login, un módulo admin, una callable de IA, una descarga de Storage, el mapa (~1-2 h). **Extras si además se va a repo privado + Access** (no incluidos arriba): los minutos de GitHub Actions dejan de ser gratis-por-público (Free: 2.000 min/mes medidos) y `ci.yml` empezaría a consumirlos; el `docs/` público se apaga; y Access añade un **segundo login** delante del de Firebase para las cuadrillas en campo — fricción permanente a cambio de cerrar la exposición del HTML.

**60.5 Anti-patterns evitados (§3)**: **NO se migró por miedo** — habría sido cambiar un riesgo bajo y reversible por una URL nueva para todo el equipo, fricción de un segundo login y una sesión de cerebro más, con `10` pidiendo producto y el costo-cerebro 30d en 54% 🔴 · **NO se declaró "cero riesgo"**: el supuesto (1) es gris de verdad y la frase de las contraseñas se incumple al pie de la letra — decirlo es lo que hace confiable el resto del ADR (§3.3) · **NO se respondió solo la pregunta literal**: auditar la cláusula de Pages sin mirar Vercel habría dejado intacto el incumplimiento MÁS claro de los tres proveedores · **NO se compró dominio** para habilitar Access: se verificó que `*.pages.dev` se puede proteger (free-tier sagrado, §3.2) · **NO se afirmó "ancho de banda ilimitado"** por repetir el marketing: no está en la doc que se leyó, y se marcó como no verificado · **NO se metió el gotcha del `.html` en `30-LECCIONES`**: esa neurona ya está ≥90% de su tope y la lección solo aplica si algún día se migra — vive aquí, enlazada desde `00` (§G.5 one-in-one-out).

**60.6 Archivos modificados/INTACTOS**: MODIFICADOS — `docs/99-HISTORIAL-ADR.md` (este §), `docs/00-INDICE.md` (fila §60), `docs/10-MEMORIA-CORTO-PLAZO.md` (bitácora). INTACTOS verificados — **todo el producto**: `index.html`, `home.html`, `pages/`, `admin/`, `assets/`, `api/`, `functions/`, `firestore.rules`, `storage.rules`, `vercel.json`, `.github/workflows/`, `package.json`, `sitemap.xml`, `robots.txt`.

**60.7 Doctrina aplicada + disparadores de reapertura**: §3.3 en cada afirmación (los ToS se leyeron de la fuente el mismo día, el sitio se probó vivo con `curl`, cada hallazgo cita archivo:línea) · §3.6 arquitecto (la decisión se tomó por costo, disponibilidad y mantenibilidad, no por la letra del contrato) · §3.2 free-tier sagrado · §G.3 consolidación (decisión cerrada → `99` + `00`, no queda tarea viva en `10`). Cache §4: n/a. **Esta decisión NO se re-analiza por calendario. Se reabre SOLO si pasa una de estas** — (a) GitHub envía cualquier aviso sobre el uso de Pages → migrar con el runbook 60.4, ya costeado; (b) la plataforma empieza a **venderse, licenciarse o alojar a un segundo operador** → entonces sí dispara el supuesto (3) "SaaS comercial" y la migración pasa de opcional a obligatoria; (c) el tráfico se acerca a los 100 GB/mes del límite blando de GitHub; (d) una auditoría de seguridad exige que la app deje de ser alcanzable públicamente (hoy el HTML de las páginas protegidas se sirve a cualquiera) → ahí Cloudflare Access es el argumento fuerte, y es de **seguridad**, no de términos de servicio. **Decisión NO revisada externamente** (§G.2 🛰️): no se convocó consejo externo ni comité — es reversible, no toca datos ni arquitectura, y su costo de error es un runbook de medio día.
---

## 61. ADR — Fichas Técnicas: de módulo suelto a página del sitio (modelo híbrido `.ftm-`) ⟦OPUS-5⟧ (2026-08-15)
...
**61.1 Causa raíz.** El módulo de Fichas Técnicas vivía como HTML suelto de 1,8 MB en una carpeta de
...
**61.2 Solución estructural.** Modelo **híbrido** (elegido por el Ingeniero entre cuatro proyectados
...
**61.3 No-regresión.** 1254 pruebas (1252 pass · 0 fail · 2 skip) + `lint:html` limpio. Ningún ID,
...
**61.4 Verificación.** Auditoría adversarial de dos agentes: `git status` sin modificados fuera del
...
**61.5 Anti-patterns evitados.** Sin `transition: all`, sin animar propiedades de layout, sin
...
**61.6 Archivos.** NUEVOS: `pages/fichas-tecnicas.html` · `assets/css/fichas-tecnicas.css` ·
...
**61.7 Doctrina aplicada.** §3.2 cambios aditivos · §3.3 evidencia antes de afirmar · §3.6 arquitecto ·
§G.4 captura. **Hallazgo de seguridad**: ver `30-LECCIONES` L-64. Sin cache bump (§4 dormida).

### 61.8 — Lo que quedó declarado, no resuelto
