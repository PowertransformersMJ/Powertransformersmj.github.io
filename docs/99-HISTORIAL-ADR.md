...
**28.10 PENDIENTE — SFRA**: el director reporta que la prueba **SFRA** (respuesta en frecuencia) "existe en los informes pero no la ve". Los SFRA son **PDFs SEPARADOS** (`240119…SFRA…estrella/delta`, `220118…SFRA`) → se cargan como informes propios. El código YA los soporta (ADR-027: familia genérica en multi-año; el selector muestra los bloques de cualquier informe). FALTA: extraer los SFRA reales a fixtures para validar el render (intento bloqueado por límite de uso del subagente). ⚠️ Posible límite real: si el dato SFRA en el PDF es solo **gráfica-imagen** (sin tabla numérica), ni la IA ni nadie puede graficar puntos sin INVENTAR → solo se podrían extraer coeficientes de correlación por banda SI están tabulados. Verificar al leer el PDF SFRA.
