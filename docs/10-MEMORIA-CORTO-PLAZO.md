...
>
> **PRÓXIMO:** Tendencia F1-F3 + biblioteca-fixes EN PRODUCCIÓN (`origin/main 64d3dab`). **Veredicto 100% NORMATIVO
> → ADR-011 (`99 §11`, L-36)**: scorecard/KPI/matriz/tendencia derivan del VALOR vs norma (ya NO del texto IA);
> aislamiento NETA por clase unificado en el dominio; **gráfica de desviación general en resistencia**. Sin commitear aún.
> ⚠️ **DECISIONES del director (normativas)**: (1) resistencia ≤5% código vs **≤2% NETA §7.2.2.D.8** → confirmar;
> (2) relación con valor `verificar` (TAP6 1.26%) hoy "fuera de norma" duro → ¿degradar a "verificar"?; (3) badges
> por bloque aún con texto IA → ¿recomputar normativamente? Luego: validar más secciones con informes reales.
>
