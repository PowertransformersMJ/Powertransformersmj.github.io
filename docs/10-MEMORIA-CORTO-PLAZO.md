...
- **2026-06-07** — **Correcciones render (feedback sobre 450108 real)**: (1) scorecard no mostraba tan δ → la IA ahora emite `prueba:"tan_delta"` (antes `tand`); FAMILIAS_SCORE aliasea ambos. (2) **`bloqueDesviacion` reescrito por física de prueba**: excitación = Δ entre las DOS fases laterales mayores (la central es menor por geometría; antes (max−min) daba ~40% falso); relación/resistencia = desviación de CADA fase vs promedio (3 líneas) contra banda ±límite. (3) **Truncación de observaciones**: el cap `TXT=240` cortaba el análisis crítico a media frase → subido a 1200 (aplica a extracciones NUEVAS; la 450108 guardada sigue cortada hasta re-cargar). 997/997. **OJO clave aprendida: la `prueba` que emite la IA NO es estable (tand/tan_delta) — aliasear.**
- **2026-06-07** — **Lote 3: prompt IA + desviación + IP (desplegado)**: prompt de la función ahora pide por bloque: `limite_desbalance` (criterio de desbalance entre fases: relación 0.5/resistencia 5/excitación 10), serie **IP** (índice de polarización R10min/R1min) en aislamiento, y **análisis crítico OBLIGATORIO** en observaciones. Schema + dominio (`sanitizarBloque`) preservan `limite_desbalance`. Render: chart derivado **"Desviación entre fases por TAP"** (`bloqueDesviacion`: Δ%=(max−min)/prom×100 vs el límite). Función re-desplegada. 997/997. **Estos datos nuevos (IP, desbalance, análisis) aparecen al RE-CARGAR 450108** (1 corrida). Render ya listo para consumirlos.
- **2026-06-07** — **Render bloques lote 2: filtro por fase**: chips A/B/C interactivos en curvas multi-serie (`renderBloque` repinta el SVG con las series activas; nunca deja 0). CSS `.pe-fase-chip`. 997/997. Pendiente: chart desviación+criterio (necesita límite por prueba → mejor vía IA), IP (R10min vía IA), concepto crítico obligatorio en prompt.
- **2026-06-07** — **Mejoras render bloques (lote 1, feedback profundo director)**: el director pide ir "más allá del informe" por bloque (filtro por fase, tablas completas, charts de valor vs desviación, IP, concepto crítico por prueba). Lote render-puro hecho: (a) **auto-rango del eje Y en curvas** (líneas zoom a [min,max]+pad en vez de aplastar contra 0 → excitación/relación/resistencia ahora muestran la variación real; límite/guía solo si caen en rango); (b) **tabla por-TAP autogenerada** de las series cuando la curva no trae tabla (`tablaDeSeries`: TAP|Fase A|B|C); (c) callout renombrado "Análisis de la IA"; (d) scorecard separa explícito **FP transformador vs FP bujes (C1)**. 997/997. **PENDIENTE (próximos lotes)**: filtro interactivo por fase; chart separado valor↕desviación+criterio (relación); IP en aislamiento (necesita la IA extraiga R10min → prompt + 1 recarga); garantizar concepto crítico por prueba en el prompt.
- **2026-06-06** — **Ajustes UX tablero (feedback director)**: (1) **libro demo seed 173523-15510 retirado** del parque — `mergeUnidades`/`mergeInformes` ya no inyectan el seed y `SERIES_SEED` vacío (todos los libros reales borrables); los exports del seed quedan como fixtures de tests (no romper). (2) **Tablero con estado vacío**: `#tablero-scope.is-empty` oculta las secciones y muestra "Ningún libro abierto" hasta seleccionar un libro (toggle en `seleccionarUnidad`). (3) **Pestañas reordenadas**: Biblioteca primero, Tablero después. 997/997, lint OK.
