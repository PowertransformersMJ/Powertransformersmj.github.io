...
- **2026-06-06** — Tablas DETALLADAS: el render mostraba solo `Año|valor|calif` por prueba, descartando fases A/B/C+terminal, TAP, par de devanados y bujes individuales que el schema SÍ guarda (esa era la "limitación" real del tablero). Nuevos renderizadores en tabla-pruebas.js (excitación/relación/resistencia/aislamiento/collar por buje) con toda la granularidad de la maqueta. Verificable en seed 173523 sin créditos.
- **2026-06-06** — IA extracción · CAUSA RAÍZ de la incompletitud hallada (debug #3, EMS 450108 seguía solo-tan-δ pese al prompt): `tool_choice` FORZADO desactiva el thinking → el modelo se conforma en docs densos. Fix desplegado: `tool_choice auto` + adaptive thinking + effort high + streaming + prompt de recorrido completo. Lección L-26. Pendiente: validar con 1 prueba (Sonnet) antes de uso masivo.
