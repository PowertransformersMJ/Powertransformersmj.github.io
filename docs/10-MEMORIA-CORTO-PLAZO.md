...
- **2026-06-07** — **Biblioteca como hub (commit `2e577a4`)**: el tab "Informes cargados" mostraba solo la unidad activa (vacío sin libro → "no carga todo"). Lo eliminé y fusioné su contenido en la Biblioteca: al abrir un libro aparece "Libro abierto · <serie>" con accesos (Ver tablero / Ver tendencia, vía `irATab`) + la lista de Informes con Abrir/Descargar PDF (`#reportlist` reubicado). `seleccionarUnidad` muestra el hub; `renderVacioSeleccion` lo oculta. **Tabs actuales: Biblioteca · Tablero · Tendencia.** Frontend. ⚠️ **DEUDA**: `10` tiene mucha bitácora del mismo día (lotes 4-8 + workflow + tendencia + biblioteca) → consolidar el arco completo del tablero a **ADR-009** + podar `10` cuando el director pause.
- **2026-06-07** — **Lote 8 (presentación, commit `78e7ec0`)**: **VERIFICADO por auditor** que la nueva corrida (JSON-2) ya trae `extra` en todas las secciones (Potencia, Tensión/Relación teórica/%DIF, R.Ref, DAR) → el workflow funcionó. Feedback del director sobre presentación: (1) `bloquesDeExtra` graficaba TODA clave `extra` → duplicados (R.Ref≈R.Medida en resistencia, %DIF/Tensión redundantes en relación); ahora `extra`→tabla y SOLO se grafica magnitud distinta (Potencia, regex `EXTRA_GRAFICABLE`). (2) Subtítulo por gráfica (`chartCap`: qué muestra + qué evalúa). (3) Tabla "Criterios de calificación" dinámica desde `CRITERIOS_NORMA` + columna Fórmula (`renderCriteriosNorma`). Solo frontend (sin deploy). Carpeta `450108/` (informe cliente) gitignored — NO publicar en Pages. **TENDENCIA temporal — Fase 1 CONSTRUIDA (commit `858e378`)**: pestaña nueva "Tendencia" + `domain/pruebas_electricas_tendencia.js` (`bloquesTendencia` puro: escalar peor-caso por prueba a lo largo de los informes vs umbral; determinista, no IA) + `renderTendenciaUI` (reusa render genérico). Con 1 informe = 1 punto; al cargar más fechas traza la tendencia. **Pendiente (Fases 2-3)**: biblioteca como timeline de informes (no solo "último"); narrativa de tendencia por IA. Decidir UX con director.
