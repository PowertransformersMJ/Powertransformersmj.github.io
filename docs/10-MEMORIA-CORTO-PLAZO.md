...
| **TODO-12** | 🛡️ **Ola 3 — CI/seguridad**. ✅ **G026/G109/G110** (commit `040f675`): `ci.yml` ahora corre los 1185 tests + se dispara en la rama de desarrollo. ✅ **G023** (`f44e0c9`): enlaces rotos a `muestras.html` → `../admin/muestras.html`. ⏳ FALTA: G025 (tests de `firestore.rules` vía emulador), G022/G094 (`lucide@latest` sin SRI en 67 pág — pin+SRI o self-host), G111 (deps: fix `websocket-driver`), G024/G019 (CSP + escapar innerHTML en dashboards). | 🟡 CI+links listos | resto pend |
| **TODO-13** | 💸 **Ola 4 — Costo/robustez**. ✅ **G015** (`33bb45a`): auditoría suministros rota (setDoc/addDoc) reparada. ✅ **G018** (`20f81e0`): `/scada_eventos` con `limit(5000)`. ⏳ FALTA: G016 (dashboard suministros suscribe 2× las mismas colecciones), G017 (validación stock no atómica en `runTransaction`), G095 (borrado de transformador sin cascada → huérfanos). | 🟡 2 de 5 | resto pend |
