...
| **TODO-07** | **Validación VISUAL del tablero (ADR-008) — lote 6 (commits `2534bcb`+`4548fb6`)**: director (1) **pushea DESARROLLO** → las tablas de TAP ganan **Desviación %/Evaluación** (DETERMINISTAS, sin re-run) y (2) **re-corre 450108** → columnas `extra` (Potencia W/tensión/relación teórica/%DIF/R.Ref) SI la IA las emite. **YO verifico por logs `[IA-DIAG]` que `extra` viene ANTES de declarar OK** (L-32/L-33). | 🔄 abierto | push + re-corrida + verificación por logs |
